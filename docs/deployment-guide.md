...
| `NEXT_PUBLIC_APP_URL` | Public URL (e.g., `https://cs-erp.codilla.ai`) |
...
- **Connection pooling:** PgBouncer at `06-build-pgbouncer-1:5432`
...
docker exec 06-build-postgres-1 pg_dump -U codilla -Fc cs_erp > backup_$(date +%Y%m%d).dump
```

### Database Restore

```bash
docker exec -i 06-build-postgres-1 pg_restore -U codilla -d cs_erp --clean backup_20260301.dump
```
...
- [ ] Health endpoint responding: `curl https://cs-erp.codilla.ai/api/health`
