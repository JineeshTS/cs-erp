...
CREATE USER cs_erp_user WITH PASSWORD 'localdevpass';
CREATE DATABASE cs_erp OWNER cs_erp_user;
GRANT ALL PRIVILEGES ON DATABASE cs_erp TO cs_erp_user;
\q
```
...
DATABASE_URL=postgresql://cs_erp_user:localdevpass@localhost:5432/cs_erp
DIRECT_DATABASE_URL=postgresql://cs_erp_user:localdevpass@localhost:5432/cs_erp
...
| `role "cs_erp_user" does not exist` | Create user: `sudo -u postgres createuser cs_erp_user` |
| `database "cs_erp" does not exist` | Create DB: `sudo -u postgres createdb -O cs_erp_user cs_erp` |
