...
export async function PATCH(
  ...
  try {
    ...
    const [updated] = await db.update(vrsVoyagePnls)
      ...
      .where(and(eq(vrsVoyagePnls.id, id), eq(vrsVoyagePnls.tenantId, user.tenantId), isNull(vrsVoyagePnls.deletedAt)))
      .returning();

    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "update", entityType: "voyage-pnls", entityId: updated?.id, module: "voyage-results-settlement", previousData: existing as Record<string, unknown>, newData: updated as Record<string, unknown>, request });
...
export async function DELETE(
  ...
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "delete", entityType: "voyage-pnls", entityId: deleted?.id, module: "voyage-results-settlement", previousData: existing as Record<string, unknown>, request });
