...
export async function PATCH(
  ...
  try {
    ...
    const [updated] = await db.update(vrsVoyageCloses)
      ...
      .where(and(eq(vrsVoyageCloses.id, id), eq(vrsVoyageCloses.tenantId, user.tenantId), isNull(vrsVoyageCloses.deletedAt)))
      .returning();

    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "update", entityType: "voyage-closes", entityId: updated.id, module: "voyage-results-settlement", previousData: existing as Record<string, unknown>, newData: updated as Record<string, unknown>, request });
...
export async function DELETE(
  ...
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "delete", entityType: "voyage-closes", entityId: deleted.id, module: "voyage-results-settlement", previousData: existing as Record<string, unknown>, request });
