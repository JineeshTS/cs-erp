...
import { createProfitBenchmarkSchema } from "@/lib/voyage-results-settlement/validation";
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("Failed to list profit benchmarks:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [record] = await db.insert(vrsProfitBenchmarks).values({
      tenantId: user.tenantId,
      benchmarkRef: `PB-${nanoid(12)}`,
      ...parsed.data,
      status: "draft",
    }).returning();
    ...
    console.error("Failed to create profit benchmark:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
