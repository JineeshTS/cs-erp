...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(or(ilike(abiTradeLaneAnalytics.analyticsRef, `%${escapeIlike(search)}%`), ilike(abiTradeLaneAnalytics.tradeLaneName, `%${escapeIlike(search)}%`))!);
    if (status) conditions.push(eq(abiTradeLaneAnalytics.status, status));
    const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(abiTradeLaneAnalytics.createdAt, abiTradeLaneAnalytics.id, parsedCursor));
