...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      conditions.push(
        or(
          ilike(abiMarketIntelligenceReports.reportRef, `%${search}%`),
          ilike(abiMarketIntelligenceReports.title, `%${search}%`),
          ilike(abiMarketIntelligenceReports.region, `%${search}%`)
        )!
      );
    }
    ...
    if (cursor) {
      conditions.push(gt(abiMarketIntelligenceReports.createdAt, new Date(cursor)));
    }
    ...
    const results = await db
      ...
      .orderBy(desc(abiMarketIntelligenceReports.createdAt))
      .limit(limit + 1);
