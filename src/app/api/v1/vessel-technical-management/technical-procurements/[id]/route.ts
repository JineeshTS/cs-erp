...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const [record] = await db.select().from(vtmTechnicalProcurements).where(and(eq(vtmTechnicalProcurements.id, id), eq(vtmTechnicalProcurements.tenantId, user.tenantId), isNull(vtmTechnicalProcurements.deletedAt))).limit(1);
    if (!record) return NextResponse.json({ error: { code: "NOT_FOUND", message: "Technical procurement not found" } }, { status: 404 });
    return NextResponse.json({ data: record });
  } catch (error) {
    console.error("Failed to get technical procurement:", error);
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error("Failed to update technical procurement:", error);
    return NextResponse.json({ error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error("Failed to delete technical procurement:", error);
    return NextResponse.json({ error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } }, { status: 500 });
  }
}
