...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(or(ilike(acmSoxFinancialControls.controlRef, `%${escapeIlike(search)}%`), ilike(acmSoxFinancialControls.title, `%${escapeIlike(search)}%`))!);
    if (status) conditions.push(eq(acmSoxFinancialControls.status, status));
    const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(acmSoxFinancialControls.createdAt, acmSoxFinancialControls.id, parsedCursor));
