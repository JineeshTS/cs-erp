...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(or(ilike(acmRiskRegisters.riskRef, `%${escapeIlike(search)}%`), ilike(acmRiskRegisters.title, `%${escapeIlike(search)}%`))!);
    if (status) conditions.push(eq(acmRiskRegisters.status, status));
    const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(acmRiskRegisters.createdAt, acmRiskRegisters.id, parsedCursor));
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "risk-registers", entityId: created.id, module: "audit-compliance-management", newData: created as Record<string, unknown>, request });
