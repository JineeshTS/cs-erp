...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      conditions.push(
        or(
          ilike(ccrTransitProcedures.transitRef, `%${search}%`),
          ilike(ccrTransitProcedures.principalName, `%${search}%`),
          ilike(ccrTransitProcedures.containerNumber, `%${search}%`)
        )!
      );
    }
    ...
    const results = await db
      ...
      .orderBy(desc(ccrTransitProcedures.createdAt))
      .limit(limit + 1);
