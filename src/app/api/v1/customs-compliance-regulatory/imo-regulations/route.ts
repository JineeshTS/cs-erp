...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      conditions.push(
        or(
          ilike(ccrImoRegulations.regulationRef, `%${escapeIlike(search)}%`),
          ilike(ccrImoRegulations.title, `%${escapeIlike(search)}%`),
          ilike(ccrImoRegulations.imoReference, `%${escapeIlike(search)}%`)
        )!
      );
    }
    ...
    if (parsedCursor) {
      conditions.push(cursorCondition(ccrImoRegulations.createdAt, ccrImoRegulations.id, parsedCursor));
    }
    ...
    const results = await db
      ...
      .orderBy(desc(ccrImoRegulations.createdAt), desc(ccrImoRegulations.id))
      .limit(limit + 1);
