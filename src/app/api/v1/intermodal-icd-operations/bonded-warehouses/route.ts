...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      conditions.push(
        or(
          ilike(icdBondedWarehouses.warehouseRef, `%${search}%`),
          ilike(icdBondedWarehouses.warehouseName, `%${search}%`),
          ilike(icdBondedWarehouses.warehouseCode, `%${search}%`)
        )!
      );
    }
    ...
    if (cursor) {
      conditions.push(lt(icdBondedWarehouses.createdAt, new Date(cursor)));
    }
    ...
    const results = await db
      ...
      .orderBy(desc(icdBondedWarehouses.createdAt))
      .limit(limit + 1);
