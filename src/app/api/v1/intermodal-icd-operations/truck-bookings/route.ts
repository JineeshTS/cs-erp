...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      conditions.push(
        or(
          ilike(icdTruckBookings.bookingRef, `%${search}%`),
          ilike(icdTruckBookings.transporterName, `%${search}%`),
          ilike(icdTruckBookings.containerNumber, `%${search}%`)
        )!
      );
    }
    ...
    if (cursor) {
      conditions.push(gt(icdTruckBookings.createdAt, new Date(cursor)));
    }
    ...
    const results = await db
      ...
      .orderBy(desc(icdTruckBookings.createdAt))
      .limit(limit + 1);
