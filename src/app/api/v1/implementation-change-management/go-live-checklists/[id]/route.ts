...
export async function PATCH(
  ...
  try {
    ...
    const [updated] = await db.update(icmGoLiveChecklists)
      ...
      .where(and(eq(icmGoLiveChecklists.id, id), eq(icmGoLiveChecklists.tenantId, user.tenantId), isNull(icmGoLiveChecklists.deletedAt)))
      .returning();

    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "update", entityType: "go-live-checklists", entityId: updated?.id, module: "implementation-change-management", previousData: existing as Record<string, unknown>, newData: updated as Record<string, unknown>, request });
...
export async function DELETE(
  ...
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "delete", entityType: "go-live-checklists", entityId: deleted?.id, module: "implementation-change-management", previousData: existing as Record<string, unknown>, request });
