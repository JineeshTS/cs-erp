...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(csoInquiries.subject, `%${escapeIlike(search)}%`));
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "inquiries", entityId: created.id, module: "customer-service-operations", newData: created as Record<string, unknown>, request });
