...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(csoEscalations.reason, `%${escapeIlike(search)}%`));
    if (status) conditions.push(eq(csoEscalations.status, status));
    const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(csoEscalations.createdAt, csoEscalations.id, parsedCursor));
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "escalations", entityId: created.id, module: "customer-service-operations", newData: created as Record<string, unknown>, request });
