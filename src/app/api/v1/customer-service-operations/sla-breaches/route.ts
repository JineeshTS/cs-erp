...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(csoSlaBreaches.breachType, `%${escapeIlike(search)}%`));
    const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(csoSlaBreaches.createdAt, csoSlaBreaches.id, parsedCursor));
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "sla-breaches", entityId: created.id, module: "customer-service-operations", newData: created as Record<string, unknown>, request });
