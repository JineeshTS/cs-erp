...
import { createContinuityPlanSchema } from "@/lib/loss-prevention-risk-management/validation";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await listContinuityPlans({
    ...
    console.error("Failed to list continuity plans:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [record] = await db
      .insert(lprContinuityPlans)
      .values({
        tenantId: user.tenantId,
        planRef: `CP-${nanoid(12)}`,
        ...parsed.data,
        status: "draft",
      })
      .returning();
    ...
    console.error("Failed to create continuity plan:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
