...
export async function GET(
  ...
  try {
    ...
    const record = await getPiClubScoring(id, user.tenantId);
    if (!record) {
      return NextResponse.json(
        { error: { code: "NOT_FOUND", message: "P&I club scoring not found" } },
        { status: 404 }
      );
    }
    ...
    console.error("Failed to get P&I club scoring:", error);
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    const { id } = await params;
    const existing = await getPiClubScoring(id, user.tenantId);
    if (!existing) {
      return NextResponse.json(
        { error: { code: "NOT_FOUND", message: "P&I club scoring not found" } },
        { status: 404 }
    ...
    const parsed = updatePiClubScoringSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    console.error("Failed to update P&I club scoring:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.error("Failed to delete P&I club scoring:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
