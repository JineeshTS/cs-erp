...
import { listEmergencyProcedures } from "@/lib/loss-prevention-risk-management/service";
import { createEmergencyProcedureSchema } from "@/lib/loss-prevention-risk-management/validation";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [record] = await db
      .insert(lprEmergencyProcedures)
      .values({
        tenantId: user.tenantId,
        procedureRef: `EP-${nanoid(12)}`,
        ...parsed.data,
        status: "draft",
      })
      .returning();

    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "emergency-procedures", entityId: record?.id, module: "loss-prevention-risk-management", newData: record as Record<string, unknown>, request });
