...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const refs = await checkActiveReferences(id, user.tenantId, MDM_REFERENCE_CHECKS.customers);
    if (refs.length > 0) {
      const details = refs.map((r) => `${r.label} (${r.count})`).join(", ");
      return NextResponse.json(
        { error: { code: "CONFLICT", message: `Cannot delete customer — active references exist: ${details}` } },
        { status: 409 }
      );
    }
