...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(customers.name, `%${escapeIlike(search)}%`));
    ...
    if (parsedCursor) conditions.push(cursorCondition(customers.createdAt, customers.id, parsedCursor));

    const results = await db.select().from(customers).where(and(...conditions))
      .orderBy(desc(customers.createdAt), desc(customers.id)).limit(limit + 1);
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "customers", entityId: created.id, module: "master-data-management", newData: created as Record<string, unknown>, request });
