...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(tariffCodes.description, `%${escapeIlike(search)}%`));
    ...
    if (parsedCursor) conditions.push(cursorCondition(tariffCodes.createdAt, tariffCodes.id, parsedCursor));

    const results = await db.select().from(tariffCodes).where(and(...conditions))
      .orderBy(desc(tariffCodes.createdAt), desc(tariffCodes.id)).limit(limit + 1);
