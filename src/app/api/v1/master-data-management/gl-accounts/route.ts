...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(glAccounts.name, `%${escapeIlike(search)}%`));
    ...
    if (parsedCursor) conditions.push(cursorCondition(glAccounts.createdAt, glAccounts.id, parsedCursor));

    const results = await db.select().from(glAccounts).where(and(...conditions))
      .orderBy(desc(glAccounts.createdAt), desc(glAccounts.id)).limit(limit + 1);
    ...
    console.error("Failed to list GL accounts:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "gl-accounts", entityId: created.id, module: "master-data-management", newData: created as Record<string, unknown>, request });
