...
import { createExchangeRateSchema } from "@/lib/master-data-management/validation";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const conditions = [eq(exchangeRates.tenantId, user.tenantId), isNull(exchangeRates.deletedAt)];
    if (search) conditions.push(ilike(exchangeRates.baseCurrency, `%${escapeIlike(search)}%`));
    ...
    if (parsedCursor) conditions.push(cursorCondition(exchangeRates.createdAt, exchangeRates.id, parsedCursor));

    const results = await db.select().from(exchangeRates).where(and(...conditions))
      .orderBy(desc(exchangeRates.createdAt), desc(exchangeRates.id)).limit(limit + 1);
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "exchange-rates", entityId: created.id, module: "master-data-management", newData: created as Record<string, unknown>, request });
