...
export async function PATCH(
  ...
  try {
    ...
    const [updated] = await db.update(thmRevenueAttributions)
      ...
      .where(and(eq(thmRevenueAttributions.id, id), eq(thmRevenueAttributions.tenantId, user.tenantId), isNull(thmRevenueAttributions.deletedAt)))
      .returning();

    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "update", entityType: "revenue-attributions", entityId: updated?.id, module: "transshipment-hub-management", previousData: existing as Record<string, unknown>, newData: updated as Record<string, unknown>, request });
...
export async function DELETE(
  ...
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "delete", entityType: "revenue-attributions", entityId: deleted?.id, module: "transshipment-hub-management", previousData: existing as Record<string, unknown>, request });
