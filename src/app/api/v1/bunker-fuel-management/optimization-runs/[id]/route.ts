...
import { updateOptimizationRunSchema } from "@/lib/bunker-fuel-management/validation";
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const record = await getOptimizationRun(id, user.tenantId);
    if (!record) {
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const parsed = updateOptimizationRunSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    const [updated] = await db
      .update(bfmOptimizationRuns)
      .set({ ...parsed.data })
      .where(
        and(
          eq(bfmOptimizationRuns.id, id),
          eq(bfmOptimizationRuns.tenantId, user.tenantId),
          isNull(bfmOptimizationRuns.deletedAt)
        )
      )
      .returning();
