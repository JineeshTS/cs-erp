...
export async function GET(
  ...
  try {
    ...
    const record = await getStorageDemurrageTariff(id, user.tenantId);
    if (!record) {
      return NextResponse.json(
        { error: { code: "NOT_FOUND", message: "Storage demurrage tariff not found" } },
        { status: 404 }
      );
    }
    ...
    console.error("Failed to get storage demurrage tariff:", error);
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    const parsed = updateStorageDemurrageTariffSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    console.error("Failed to update storage demurrage tariff:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.error("Failed to delete storage demurrage tariff:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
