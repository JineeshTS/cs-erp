...
export async function GET(request: NextRequest) {
  try {
    ...
    const conditions = [
      eq(eqyContainerFleet.tenantId, user.tenantId),
      isNull(eqyContainerFleet.deletedAt),
    ];
    if (search) conditions.push(ilike(eqyContainerFleet.containerNumber, `%${search}%`));
    if (status) conditions.push(eq(eqyContainerFleet.status, status));
    if (cursor) conditions.push(gt(eqyContainerFleet.createdAt, new Date(cursor)));
    ...
    console.error("Failed to list container fleet:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "equipment-control-yard-managem", entityId: created?.id, module: "equipment-control-yard-managem", newData: created as Record<string, unknown>, request });
