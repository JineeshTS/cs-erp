...
export async function GET(request: NextRequest) {
  try {
    ...
    if (cursor) conditions.push(lt(eqyContainerFleet.createdAt, new Date(cursor)));

    const results = await db.select().from(eqyContainerFleet).where(and(...conditions)).orderBy(desc(eqyContainerFleet.createdAt)).limit(limit + 1);
