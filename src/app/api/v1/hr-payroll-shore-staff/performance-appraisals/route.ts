...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      conditions.push(
        or(
          ilike(hpsPerformanceAppraisals.appraisalRef, `%${escapeIlike(search)}%`),
          ilike(hpsPerformanceAppraisals.employeeName, `%${escapeIlike(search)}%`)
        )!
      );
    }
    ...
    if (parsedCursor) {
      conditions.push(cursorCondition(hpsPerformanceAppraisals.createdAt, hpsPerformanceAppraisals.id, parsedCursor));
    }
    ...
    const results = await db
      ...
      .orderBy(desc(hpsPerformanceAppraisals.createdAt), desc(hpsPerformanceAppraisals.id))
      .limit(limit + 1);
