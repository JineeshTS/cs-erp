...
export async function GET(request: NextRequest) {
  try {
    ...
    if (parsedCursor) {
      conditions.push(cursorCondition(hpsEmployeeProfiles.createdAt, hpsEmployeeProfiles.id, parsedCursor));
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "employee-profiles", entityId: created.id, module: "hr-payroll-shore-staff", newData: created as Record<string, unknown>, request });
