...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(melsLegalEntities.name, `%${escapeIlike(search)}%`));
    ...
    if (parsedCursor) conditions.push(cursorCondition(melsLegalEntities.createdAt, melsLegalEntities.id, parsedCursor));

    const results = await db.select().from(melsLegalEntities).where(and(...conditions))
      .orderBy(desc(melsLegalEntities.createdAt), desc(melsLegalEntities.id)).limit(limit + 1);
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "legal-entities", entityId: created.id, module: "multi-entity-legal-structure", newData: created as Record<string, unknown>, request });
