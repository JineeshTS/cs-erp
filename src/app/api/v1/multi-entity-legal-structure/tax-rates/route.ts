...
export async function GET(request: NextRequest) {
  try {
    ...
    if (parsedCursor) conditions.push(cursorCondition(melsTaxRates.createdAt, melsTaxRates.id, parsedCursor));

    const results = await db.select().from(melsTaxRates).where(and(...conditions))
      .orderBy(desc(melsTaxRates.createdAt), desc(melsTaxRates.id)).limit(limit + 1);
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "tax-rates", entityId: created.id, module: "multi-entity-legal-structure", newData: created as Record<string, unknown>, request });
