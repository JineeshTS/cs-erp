...
export async function GET(
  ...
  try {
    ...
    const record = await getNetworkDesign(id, user.tenantId);
    if (!record) {
    ...
    console.error("Failed to get network design:", error);
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    const { id } = await params;
    const existing = await getNetworkDesign(id, user.tenantId);
    if (!existing) {
      return NextResponse.json(
        { error: { code: "NOT_FOUND", message: "Not found" } },
        { status: 404 }
