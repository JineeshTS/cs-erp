...
import { createDeploymentContractSchema } from "@/lib/fleet-deployment-planning/validation";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await listDeploymentContracts({
    ...
    console.error("Failed to list deployment contracts:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [record] = await db
      .insert(fdpDeploymentContracts)
      .values({
        tenantId: user.tenantId,
        contractRef: `DC-${nanoid(12)}`,
        ...parsed.data,
        status: "draft",
      })
      .returning();
