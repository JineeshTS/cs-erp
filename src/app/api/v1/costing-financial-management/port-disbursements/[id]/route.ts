...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const record = await getPortDisbursement(id, user.tenantId);

    if (!record) return NextResponse.json({ error: { code: "NOT_FOUND", message: "Port disbursement not found" } }, { status: 404 });
    return NextResponse.json({ data: record });
  } catch (error) {
    console.error("Failed to get port disbursement:", error);
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error("Failed to update port disbursement:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error("Failed to delete port disbursement:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
