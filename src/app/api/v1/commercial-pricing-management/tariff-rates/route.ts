...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(cpmTariffRates.chargeName, `%${escapeIlike(search)}%`));
    const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(cpmTariffRates.createdAt, cpmTariffRates.id, parsedCursor));

    const results = await db.select().from(cpmTariffRates).where(and(...conditions)).orderBy(desc(cpmTariffRates.createdAt), desc(cpmTariffRates.id)).limit(limit + 1);
