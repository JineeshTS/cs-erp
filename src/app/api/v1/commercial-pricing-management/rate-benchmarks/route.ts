...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(cpmRateBenchmarks.benchmarkName, `%${escapeIlike(search)}%`));
    const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(cpmRateBenchmarks.createdAt, cpmRateBenchmarks.id, parsedCursor));

    const results = await db.select().from(cpmRateBenchmarks).where(and(...conditions)).orderBy(desc(cpmRateBenchmarks.createdAt), desc(cpmRateBenchmarks.id)).limit(limit + 1);
