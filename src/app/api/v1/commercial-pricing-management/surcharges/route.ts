...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(cpmSurcharges.surchargeName, `%${escapeIlike(search)}%`));
    const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(cpmSurcharges.createdAt, cpmSurcharges.id, parsedCursor));

    const results = await db.select().from(cpmSurcharges).where(and(...conditions)).orderBy(desc(cpmSurcharges.createdAt), desc(cpmSurcharges.id)).limit(limit + 1);
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "surcharges", entityId: created.id, module: "commercial-pricing-management", newData: created as Record<string, unknown>, request });
