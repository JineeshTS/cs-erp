...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(cpmProfitabilityAnalyses.analysisName, `%${escapeIlike(search)}%`));
    if (status) conditions.push(eq(cpmProfitabilityAnalyses.status, status));
    const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(cpmProfitabilityAnalyses.createdAt, cpmProfitabilityAnalyses.id, parsedCursor));

    const results = await db.select().from(cpmProfitabilityAnalyses).where(and(...conditions)).orderBy(desc(cpmProfitabilityAnalyses.createdAt), desc(cpmProfitabilityAnalyses.id)).limit(limit + 1);
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "profitability-analyses", entityId: created.id, module: "commercial-pricing-management", newData: created as Record<string, unknown>, request });
