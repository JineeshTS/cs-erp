...
export async function POST(
  ...
  try {
    ...
    const cloned = await cloneProcessDefinition(user.tenantId, user.id, id);
    ...
    void logBusinessAudit({
      ...
      newData: {
        ...
        name: cloned.name,
        source: "cloned",
        processCode: cloned.processCode,
      },
      request,
    });
    ...
    console.error("Failed to clone process definition:", error);
    return NextResponse.json(
      {
        error: {
          code: "INTERNAL_ERROR",
