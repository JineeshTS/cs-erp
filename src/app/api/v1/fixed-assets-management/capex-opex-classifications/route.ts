...
import { createCapexOpexClassificationSchema } from "@/lib/fixed-assets-management/validation";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await listCapexOpexClassifications({
    ...
    console.error("Failed to list capex/opex classifications:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [record] = await db
      .insert(famCapexOpexClassifications)
      .values({
        ...parsed.data,
        classificationRef: `FCO-${Date.now()}`,
        tenantId: user.tenantId,
        status: "draft",
      })
      .returning();
    ...
    console.error("Failed to create capex/opex classification:", error);
    return NextResponse.json(
      {
        error: {
          code: "INTERNAL_ERROR",
