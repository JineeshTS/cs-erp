...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(capLoadingLists.listReference, `%${escapeIlike(search)}%`));
    if (status) conditions.push(eq(capLoadingLists.status, status));
    const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(capLoadingLists.createdAt, capLoadingLists.id, parsedCursor));
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "loading-lists", entityId: created.id, module: "capacity-voyage-management", newData: created as Record<string, unknown>, request });
