...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) { conditions.push(or(ilike(simHireSurveys.surveyRef, `%${escapeIlike(search)}%`), ilike(simHireSurveys.vesselName, `%${escapeIlike(search)}%`), ilike(simHireSurveys.chartererName, `%${escapeIlike(search)}%`))!); }
    if (status) { conditions.push(eq(simHireSurveys.status, status)); }
    if (cursor) { const parsedCursor = parseCompoundCursor(cursor);
      if (parsedCursor) conditions.push(cursorCondition(simHireSurveys.createdAt, simHireSurveys.id, parsedCursor)); }
