...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      conditions.push(
        or(
          ilike(simSurveyReports.reportRef, `%${escapeIlike(search)}%`),
          ilike(simSurveyReports.title, `%${escapeIlike(search)}%`),
          ilike(simSurveyReports.vesselName, `%${escapeIlike(search)}%`)
        )!
      );
    }
    ...
    if (parsedCursor) {
      conditions.push(cursorCondition(simSurveyReports.createdAt, simSurveyReports.id, parsedCursor));
    }
    ...
    const results = await db
      ...
      .orderBy(desc(simSurveyReports.createdAt), desc(simSurveyReports.id))
      .limit(limit + 1);
