...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      conditions.push(
        or(
          ilike(simContainerSurveys.surveyRef, `%${escapeIlike(search)}%`),
          ilike(simContainerSurveys.containerNumber, `%${escapeIlike(search)}%`),
          ilike(simContainerSurveys.depotName, `%${escapeIlike(search)}%`)
        )!
      );
    }
    ...
    if (parsedCursor) {
      conditions.push(cursorCondition(simContainerSurveys.createdAt, simContainerSurveys.id, parsedCursor));
    }
    ...
    const results = await db
      ...
      .orderBy(desc(simContainerSurveys.createdAt), desc(simContainerSurveys.id))
      .limit(limit + 1);
