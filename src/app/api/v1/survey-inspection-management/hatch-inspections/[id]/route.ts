...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const [record] = await db.select().from(simHatchInspections).where(and(eq(simHatchInspections.id, id), eq(simHatchInspections.tenantId, user.tenantId), isNull(simHatchInspections.deletedAt)));
    if (!record) return NextResponse.json({ error: { code: "NOT_FOUND", message: "Hatch inspection not found" } }, { status: 404 });
    return NextResponse.json({ data: record });
  } catch (error) {
    console.error("Failed to get hatch inspection:", error);
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error("Failed to update hatch inspection:", error);
    return NextResponse.json({ error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error("Failed to delete hatch inspection:", error);
    return NextResponse.json({ error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } }, { status: 500 });
  }
}
