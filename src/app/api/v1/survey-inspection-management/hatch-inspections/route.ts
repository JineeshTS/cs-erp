...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) { conditions.push(or(ilike(simHatchInspections.inspectionRef, `%${escapeIlike(search)}%`), ilike(simHatchInspections.vesselName, `%${escapeIlike(search)}%`), ilike(simHatchInspections.holdNumber, `%${escapeIlike(search)}%`))!); }
    if (status) { conditions.push(eq(simHatchInspections.status, status)); }
    if (cursor) { const parsedCursor = parseCompoundCursor(cursor);
      if (parsedCursor) conditions.push(cursorCondition(simHatchInspections.createdAt, simHatchInspections.id, parsedCursor)); }
