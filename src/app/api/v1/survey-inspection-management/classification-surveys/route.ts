...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      conditions.push(
        or(
          ilike(simClassificationSurveys.surveyRef, `%${escapeIlike(search)}%`),
          ilike(simClassificationSurveys.vesselName, `%${escapeIlike(search)}%`),
          ilike(simClassificationSurveys.classificationSociety, `%${escapeIlike(search)}%`)
        )!
      );
    }
    ...
    if (parsedCursor) {
      conditions.push(cursorCondition(simClassificationSurveys.createdAt, simClassificationSurveys.id, parsedCursor));
    }
    ...
    const results = await db
      ...
      .orderBy(desc(simClassificationSurveys.createdAt), desc(simClassificationSurveys.id))
      .limit(limit + 1);
