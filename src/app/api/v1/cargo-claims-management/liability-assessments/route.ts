...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await listLiabilityAssessments({ tenantId: user.tenantId, search, status, cursor, limit });
    return NextResponse.json({ data: result.data, meta: result.meta });
  } catch (error) {
    console.error("Failed to list liability assessments:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [record] = await db
      .insert(ccmLiabilityAssessments)
      .values({
        ...parsed.data,
        assessmentRef: `CLA-${Date.now()}`,
