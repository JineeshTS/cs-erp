...
export async function GET(
  ...
  try {
    ...
    const record = await getTimeBarTracking(id, user.tenantId);
    if (!record) {
      return NextResponse.json(
        { error: { code: "NOT_FOUND", message: "Time bar tracking not found" } },
        { status: 404 }
      );
    }
    ...
    console.error("Failed to get time bar tracking:", error);
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    const parsed = updateTimeBarTrackingSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    console.error("Failed to update time bar tracking:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.error("Failed to delete time bar tracking:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
