...
export async function GET(
  ...
  try {
    ...
    const record = await getDamageSurvey(id, user.tenantId);
    if (!record) {
    ...
    console.error("Failed to get damage survey:", error);
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    const [record] = await db
      ...
      .where(
        and(
          eq(ccmDamageSurveys.id, id),
          eq(ccmDamageSurveys.tenantId, user.tenantId)
        )
      )
      .returning();
    if (!record) {
    ...
    console.error("Failed to update damage survey:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.error("Failed to delete damage survey:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
