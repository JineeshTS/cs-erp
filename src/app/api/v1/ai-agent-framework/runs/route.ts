...
import { createAgentRunSchema } from "@/lib/ai-agent-framework/validation";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(aafAgentRuns.runNumber, `%${escapeIlike(search)}%`));
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "runs", entityId: created.id, module: "ai-agent-framework", newData: created as Record<string, unknown>, request });
