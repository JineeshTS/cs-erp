...
import { createWorkflowDefinitionSchema } from "@/lib/ai-agent-framework/validation";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = createWorkflowDefinitionSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    const [created] = await db.insert(aafWorkflowDefinitions).values({
      tenantId: user.tenantId,
      ...parsed.data,
    }).returning();

    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "workflow-definitions", entityId: created?.id, module: "ai-agent-framework", newData: created as Record<string, unknown>, request });
