...
import { createFeatureConfigSchema } from "@/lib/admin-portal/validation";
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("Failed to list feature configs:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = createFeatureConfigSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    const [created] = await db.insert(adminFeatureConfigs).values({
      tenantId: user.tenantId,
      ...parsed.data,
    }).returning();

    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "feature-configs", entityId: created?.id, module: "admin-portal", newData: created as Record<string, unknown>, request });

    return NextResponse.json({ data: created }, { status: 201 });
  } catch (error) {
    console.error("Failed to create feature config:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
