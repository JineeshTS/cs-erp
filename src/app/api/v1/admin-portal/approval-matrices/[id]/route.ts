...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const [record] = await db.select().from(adminApprovalMatrices)
      .where(and(eq(adminApprovalMatrices.id, id), eq(adminApprovalMatrices.tenantId, user.tenantId), isNull(adminApprovalMatrices.deletedAt))).limit(1);

    if (!record) return NextResponse.json({ error: { code: "NOT_FOUND", message: "Approval matrix not found" } }, { status: 404 });
    return NextResponse.json({ data: record });
  } catch (error) {
    console.error("Failed to get approval matrix:", error);
