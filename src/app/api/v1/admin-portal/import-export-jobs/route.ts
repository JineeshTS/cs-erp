...
import { createImportExportJobSchema } from "@/lib/admin-portal/validation";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (cursor) conditions.push(gt(adminImportExportJobs.createdAt, new Date(cursor)));
    ...
    console.error("Failed to list import/export jobs:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [created] = await db.insert(adminImportExportJobs).values({
    ...
    console.error("Failed to create import/export job:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
