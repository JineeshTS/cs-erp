...
export async function GET(request: NextRequest) {
  try {
    ...
    if (parsedCursor) conditions.push(cursorCondition(adminIntegrationEndpoints.createdAt, adminIntegrationEndpoints.id, parsedCursor));

    const results = await db.select().from(adminIntegrationEndpoints).where(and(...conditions))
      .orderBy(desc(adminIntegrationEndpoints.createdAt), desc(adminIntegrationEndpoints.id)).limit(limit + 1);
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "integration-endpoints", entityId: created.id, module: "admin-portal", newData: created as Record<string, unknown>, request });
