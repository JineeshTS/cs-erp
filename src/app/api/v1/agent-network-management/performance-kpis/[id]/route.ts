...
export async function GET(
  ...
  try {
    ...
    const record = await getPerformanceKpi(id, user.tenantId);
    if (!record) {
      return NextResponse.json(
        { error: { code: "NOT_FOUND", message: "Performance KPI not found" } },
        { status: 404 }
      );
    }
    return NextResponse.json({ data: record });
  } catch (error) {
    console.error("Failed to get performance KPI:", error);
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    const { id } = await params;
    const existing = await getPerformanceKpi(id, user.tenantId);
    if (!existing) {
      return NextResponse.json(
        { error: { code: "NOT_FOUND", message: "Performance KPI not found" } },
        { status: 404 }
    ...
    const parsed = updatePerformanceKpiSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    console.error("Failed to update performance KPI:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.error("Failed to delete performance KPI:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
