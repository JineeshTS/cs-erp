...
import { updatePortalConfigSchema } from "@/lib/agent-network-management/validation";
...
export async function GET(
  ...
  try {
    ...
    const record = await getPortalConfig(id, user.tenantId);
    if (!record) {
      return NextResponse.json(
        { error: { code: "NOT_FOUND", message: "Portal config not found" } },
        { status: 404 }
      );
    }
...
export async function PATCH(
  ...
  try {
    ...
    const { id } = await params;
    const existing = await getPortalConfig(id, user.tenantId);
    if (!existing) {
      return NextResponse.json(
        { error: { code: "NOT_FOUND", message: "Portal config not found" } },
        { status: 404 }
    ...
    const [updated] = await db.update(anmPortalConfigs)
      .set({ ...parsed.data, updatedAt: new Date() })
      .where(and(eq(anmPortalConfigs.id, id), eq(anmPortalConfigs.tenantId, user.tenantId), isNull(anmPortalConfigs.deletedAt)))
      .returning();
...
export async function DELETE(
  ...
  try {
    ...
    const [deleted] = await db.update(anmPortalConfigs)
    ...
    console.error("Failed to delete portal config:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "An unexpected error occurred" } },
      { status: 500 }
    );
