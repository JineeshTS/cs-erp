...
import { updateDdmDisputeSchema } from "@/lib/demurrage-detention-management/validation";
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const [record] = await db
      ...
      .where(
        and(
          eq(ddmDisputes.id, id),
          eq(ddmDisputes.tenantId, user.tenantId),
          isNull(ddmDisputes.deletedAt)
        )
      )
      .limit(1);
    ...
    console.error("Failed to get D&D dispute:", error);
    return NextResponse.json(
      {
        error: {
          code: "INTERNAL_ERROR",
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const parsed = updateDdmDisputeSchema.safeParse(body);
    if (!parsed.success)
      return NextResponse.json(
        {
          error: {
