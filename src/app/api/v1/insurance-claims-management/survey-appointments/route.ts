...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      conditions.push(
        or(
          ilike(icmSurveyAppointments.appointmentRef, `%${search}%`),
          ilike(icmSurveyAppointments.surveyorName, `%${search}%`),
          ilike(icmSurveyAppointments.vesselName, `%${search}%`)
        )!
      );
    }
    ...
    if (cursor) {
      conditions.push(gt(icmSurveyAppointments.createdAt, new Date(cursor)));
    }
    ...
    const results = await db
      ...
      .orderBy(desc(icmSurveyAppointments.createdAt))
      .limit(limit + 1);
