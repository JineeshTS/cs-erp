...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      conditions.push(
        or(
          ilike(icmClaimsRegistrations.claimRef, `%${escapeIlike(search)}%`),
          ilike(icmClaimsRegistrations.claimantName, `%${escapeIlike(search)}%`),
          ilike(icmClaimsRegistrations.vesselName, `%${escapeIlike(search)}%`)
        )!
      );
    }
    ...
    if (parsedCursor) {
      conditions.push(cursorCondition(icmClaimsRegistrations.createdAt, icmClaimsRegistrations.id, parsedCursor));
    }
    ...
    const results = await db
      ...
      .orderBy(desc(icmClaimsRegistrations.createdAt), desc(icmClaimsRegistrations.id))
      .limit(limit + 1);
