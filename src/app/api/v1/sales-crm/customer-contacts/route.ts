...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(scmCustomerContacts.firstName, `%${escapeIlike(search)}%`));
    ...
    const results = await db.select().from(scmCustomerContacts).where(and(...conditions)).orderBy(desc(scmCustomerContacts.createdAt), desc(scmCustomerContacts.id)).limit(limit + 1);
