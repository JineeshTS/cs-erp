...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(scmSalesTargets.targetName, `%${escapeIlike(search)}%`));
    ...
    const results = await db.select().from(scmSalesTargets).where(and(...conditions))
      .orderBy(desc(scmSalesTargets.createdAt), desc(scmSalesTargets.id)).limit(limit + 1);
