...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(scmPipelineStages.stageName, `%${escapeIlike(search)}%`));
    const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(scmPipelineStages.createdAt, scmPipelineStages.id, parsedCursor));

    const results = await db.select().from(scmPipelineStages).where(and(...conditions)).orderBy(desc(scmPipelineStages.createdAt), desc(scmPipelineStages.id)).limit(limit + 1);
