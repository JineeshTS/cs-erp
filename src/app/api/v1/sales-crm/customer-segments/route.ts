...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) conditions.push(ilike(scmCustomerSegments.segmentName, `%${escapeIlike(search)}%`));
    const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(scmCustomerSegments.createdAt, scmCustomerSegments.id, parsedCursor));

    const results = await db.select().from(scmCustomerSegments).where(and(...conditions)).orderBy(desc(scmCustomerSegments.createdAt), desc(scmCustomerSegments.id)).limit(limit + 1);
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logBusinessAudit({ tenantId: user.tenantId, userId: user.id, userEmail: user.email, action: "create", entityType: "customer-segments", entityId: created.id, module: "sales-crm", newData: created as Record<string, unknown>, request });
