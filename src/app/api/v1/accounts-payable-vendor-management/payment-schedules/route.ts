...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await listPaymentSchedules({ tenantId: user.tenantId, search, status, cursor, limit });
    return NextResponse.json(result);
  } catch (error) {
    console.error("Failed to list payment schedule:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [created] = await db.insert(apvmPaymentSchedules).values({
