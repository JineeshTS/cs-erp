...
export default async function PortDuesWharfagesPage({
  searchParams,
}: {
  ...
  const result = await listPortDuesWharfages({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage port dues, wharfage, anchorage, and berth hire calculations
          </p>
        </div>
        {canCreate && (
          <Link
            href="/port-tariff-terminal-billing/port-dues-wharfages/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by dues ref or port name..."
              ...
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      No port dues or wharfages found
                    </p>
                  </div>
                </td>
              </tr>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/port-tariff-terminal-billing/port-dues-wharfages/${row.id}`}
                  ...
                  <td className="px-4 py-3 capitalize">
                    {row.duesType?.replace(/_/g, " ")}
                  </td>
                  <td className="px-4 py-3">{row.vesselName ?? "\u2014"}</td>
                  <td className="px-4 py-3 text-end">
                    {row.calculatedAmount ?? "\u2014"} {row.duesCurrency ?? ""}
                  </td>
                  <td className="px-4 py-3">
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/port-tariff-terminal-billing/port-dues-wharfages?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
