...
export default async function InvoiceValidationsPage({
  searchParams,
}: {
  ...
  const result = await listInvoiceValidations({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage terminal invoice validations and disputes
          </p>
        </div>
        {canCreate && (
          <Link
            href="/port-tariff-terminal-billing/invoice-validations/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by validation ref or terminal name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-start font-medium">Terminal</th>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/port-tariff-terminal-billing/invoice-validations/${row.id}`}
                  ...
                  <td className="px-4 py-3 capitalize">
                    {row.validationType?.replace(/_/g, " ") ?? "\u2014"}
                  </td>
                  <td className="px-4 py-3">
                    {row.invoiceNumber ?? "\u2014"}
                  </td>
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/port-tariff-terminal-billing/invoice-validations?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
