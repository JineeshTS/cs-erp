...
import { listCostOptimizations } from "@/lib/port-tariff-terminal-billing/service";
import { Badge } from "@/components/ui/badge";

export default async function CostOptimizationsPage({
  searchParams,
}: {
  ...
  const result = await listCostOptimizations({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage AI port cost optimization recommendations
          </p>
        </div>
        {canCreate && (
          <Link
            href="/port-tariff-terminal-billing/cost-optimizations/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by optimization ref or port name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-end font-medium">Savings</th>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/port-tariff-terminal-billing/cost-optimizations/${row.id}`}
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/port-tariff-terminal-billing/cost-optimizations?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
