...
import { listStorageDemurrageTariffs } from "@/lib/port-tariff-terminal-billing/service";
import { Badge } from "@/components/ui/badge";

export default async function StorageDemurrageTariffsPage({
  searchParams,
}: {
  ...
  const result = await listStorageDemurrageTariffs({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">
            Storage &amp; Demurrage Tariffs
          </h1>
          <p className="text-sm text-muted-foreground">
            Manage storage and demurrage tariff schedules
          </p>
        </div>
        {canCreate && (
          <Link
            href="/port-tariff-terminal-billing/storage-demurrage-tariffs/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by tariff ref or port name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-end font-medium">Free Days</th>
              ...
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      No storage or demurrage tariffs found
                    </p>
                  </div>
                </td>
              </tr>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/port-tariff-terminal-billing/storage-demurrage-tariffs/${row.id}`}
                  ...
                  <td className="px-4 py-3 capitalize">
                    {row.tariffType?.replace(/_/g, " ")}
                  </td>
                  <td className="px-4 py-3">
                    {row.containerSize
                      ? `${row.containerSize}${row.containerType ? ` / ${row.containerType}` : ""}`
                      : "\u2014"}
                  </td>
                  <td className="px-4 py-3 text-end">
                    {row.freeDays != null ? String(row.freeDays) : "\u2014"}
                  </td>
                  <td className="px-4 py-3">
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/port-tariff-terminal-billing/storage-demurrage-tariffs?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
