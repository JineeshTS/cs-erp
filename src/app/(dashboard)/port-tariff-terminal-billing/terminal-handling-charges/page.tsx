...
export default async function TerminalHandlingChargesPage({
  searchParams,
}: {
  ...
  const result = await listTerminalHandlingCharges({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage THC rates by port, terminal, and container type
          </p>
        </div>
        {canCreate && (
          <Link
            href="/port-tariff-terminal-billing/terminal-handling-charges/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by charge ref or port name..."
              ...
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      No terminal handling charges found
                    </p>
                  </div>
                </td>
              </tr>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/port-tariff-terminal-billing/terminal-handling-charges/${row.id}`}
                  ...
                  <td className="px-4 py-3 capitalize">
                    {row.chargeType?.replace(/_/g, " ")}
                  </td>
                  <td className="px-4 py-3">
                    {row.containerType ?? ""} {row.containerSize ?? ""}
                  </td>
                  <td className="px-4 py-3 text-end">
                    {row.chargeAmountBase ?? "\u2014"} {row.chargeCurrency ?? ""}
                  </td>
                  <td className="px-4 py-3">
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/port-tariff-terminal-billing/terminal-handling-charges?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
