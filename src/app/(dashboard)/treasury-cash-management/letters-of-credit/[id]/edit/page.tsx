...
export default async function EditLetterOfCreditPage({
  ...
  const initialData: Record<string, unknown> = {
    lcType: record.lcType ?? "",
    issuingBank: record.issuingBank ?? "",
    advisingBank: record.advisingBank ?? "",
    confirmingBank: record.confirmingBank ?? "",
    applicant: record.applicant ?? "",
    beneficiary: record.beneficiary ?? "",
    lcAmount: record.lcAmount ?? "",
    ...
    expiryDate: record.expiryDate
      ...
      : "",
    shipmentDeadline: record.shipmentDeadline
      ? new Date(record.shipmentDeadline).toISOString()
      : "",
    presentationPeriod: record.presentationPeriod ?? "",
    partialShipment: record.partialShipment ?? false,
    transshipment: record.transshipment ?? false,
    termsAndConditions: record.termsAndConditions ?? "",
    utilizationAmount: record.utilizationAmount ?? "",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/treasury-cash-management/letters-of-credit/${record.id}`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <TcmForm
          ...
          apiPath={`/api/v1/treasury-cash-management/letters-of-credit/${record.id}`}
          fields={LC_FIELDS}
          initialData={initialData}
          isEdit
          returnPath={`/treasury-cash-management/letters-of-credit/${record.id}`}
        />
      </div>
    </div>
  );
