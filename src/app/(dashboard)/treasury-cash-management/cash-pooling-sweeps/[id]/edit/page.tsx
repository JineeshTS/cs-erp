  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/treasury-cash-management/cash-pooling-sweeps/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">
          Edit Cash Pooling Sweep
        </h1>
      </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        <TcmForm
          ...
          apiPath={`/api/v1/treasury-cash-management/cash-pooling-sweeps/${id}`}
          fields={SWEEP_FIELDS}
          initialData={{
            sweepType: record.sweepType ?? "",
            poolName: record.poolName ?? "",
            masterAccountRef: record.masterAccountRef ?? "",
            sweepDirection: record.sweepDirection ?? "",
            triggerBalance: record.triggerBalance ?? "",
            targetBalance: record.targetBalance ?? "",
            sweepAmount: record.sweepAmount ?? "",
            currency: record.currency ?? "",
            frequency: record.frequency ?? "",
            lastExecutedAt: record.lastExecutedAt
              ? new Date(record.lastExecutedAt).toISOString()
              : "",
            nextScheduledAt: record.nextScheduledAt
              ? new Date(record.nextScheduledAt).toISOString()
              : "",
            interestRate: record.interestRate ?? "",
            totalPoolBalance: record.totalPoolBalance ?? "",
            notes: record.notes ?? "",
          }}
          isEdit
          returnPath={`/treasury-cash-management/cash-pooling-sweeps/${id}`}
        />
      </div>
    </div>
  );
