...
export default async function OrchestrationTasksListPage({
  searchParams,
}: {
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/ai-agent-framework/orchestration-tasks?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage multi-agent orchestration task execution
          </p>
        </div>
        {canCreate && (
          <Link
            href="/ai-agent-framework/orchestration-tasks/new"
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No orchestration tasks found.</p>
          {canCreate && (
            <Link
              href="/ai-agent-framework/orchestration-tasks/new"
              className="mt-3 inline-block text-sm text-blue-600 hover:underline"
            >
              Create your first orchestration task
            </Link>
          )}
        </div>
      ) : (
              ...
              {items.map((t) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/ai-agent-framework/orchestration-tasks/${t.id}`}
                      className="font-medium text-gray-900 hover:underline"
                    >
                      {t.taskName}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-gray-600">{t.taskCode}</td>
                  <td className="px-4 py-3 text-gray-600">{t.strategy}</td>
                  <td className="px-4 py-3">
