...
export default async function WorkflowDefinitionsPage({
  ...
  if (search) {
    conditions.push(
      ilike(aafWorkflowDefinitions.workflowName, `%${escapeIlike(search)}%`)
    );
  }
  const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) {
      conditions.push(cursorCondition(aafWorkflowDefinitions.createdAt, aafWorkflowDefinitions.id, parsedCursor));
    }
  ...
  const data = await db
    ...
    .orderBy(desc(aafWorkflowDefinitions.createdAt), desc(aafWorkflowDefinitions.id))
    .limit(limit + 1);
