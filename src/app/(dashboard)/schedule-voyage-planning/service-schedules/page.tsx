...
export default async function ServiceSchedulesListPage({
  searchParams,
}: {
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/schedule-voyage-planning/service-schedules?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Liner service schedule and voyage planning records</p>
        </div>
        {canCreate && (
          <Link href="/schedule-voyage-planning/service-schedules/new" className="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
            <Plus className="h-4 w-4" /> New Service Schedule
          </Link>
        )}
      </div>
      ...
      <form className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        ...
        {(search || status) && (
          <Link href="/schedule-voyage-planning/service-schedules" className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Clear</Link>
        )}
      </form>
