...
export default async function EditRoutingRulePage({
  params,
}: {
  ...
  if (!(await hasPermission(session.id, session.tenantId, "routing:edit")))
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/workflow-notification-engine/routing-rules/${id}`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <WneForm
          ...
          apiPath={`/api/v1/workflow-notification-engine/routing-rules/${id}`}
          ...
          initialData={{
            ...
            assignmentValue: rule.assignmentValue,
            fallbackAssignment: rule.fallbackAssignment ?? "",
            isActive: rule.isActive,
          }}
          isEdit
          returnPath={`/workflow-notification-engine/routing-rules/${id}`}
        />
      </div>
    </div>
  );
