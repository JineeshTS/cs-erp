...
export default async function EditWorkflowPage({
  ...
  if (!(await hasPermission(session.id, session.tenantId, "workflows:edit")))
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/workflow-notification-engine/workflows/${id}`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <WneForm
          entityType="Workflow"
          apiPath={`/api/v1/workflow-notification-engine/workflows/${id}`}
          ...
          isEdit
          returnPath={`/workflow-notification-engine/workflows/${id}`}
        />
      </div>
    </div>
  );
