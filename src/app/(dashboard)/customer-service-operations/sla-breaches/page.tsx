...
export default async function SLABreachesPage({
  ...
  if (search) {
    conditions.push(ilike(csoSlaBreaches.breachType, `%${search}%`));
  }
  if (cursor) {
    conditions.push(lt(csoSlaBreaches.createdAt, new Date(cursor)));
  }
  ...
  const data = await db
    ...
    .orderBy(desc(csoSlaBreaches.createdAt))
    .limit(limit + 1);
