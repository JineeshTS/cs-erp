...
export default async function ServiceRequestsListPage({
  ...
  if (search) conditions.push(ilike(csoServiceRequests.subject, `%${escapeIlike(search)}%`));
  const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(csoServiceRequests.createdAt, csoServiceRequests.id, parsedCursor));

  const data = await db
    .select()
    .from(csoServiceRequests)
    .where(and(...conditions))
    .orderBy(desc(csoServiceRequests.createdAt), desc(csoServiceRequests.id))
    .limit(limit + 1);
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/customer-service-operations/service-requests?${p.toString()}`;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage customer service requests, tracking, and fulfillment
          </p>
        </div>
        {canCreate && (
          <Link
            href="/customer-service-operations/service-requests/new"
      ...
      <form className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        ...
        {search && (
          <Link href="/customer-service-operations/service-requests" className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
            Clear
          </Link>
        )}
      </form>
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No service requests found.</p>
          {canCreate && (
            <Link href="/customer-service-operations/service-requests/new" className="mt-3 inline-block text-sm text-blue-600 hover:underline">
              ...
              {items.map((t) => (
                <tr key={t.id} className="border-b last:border-0 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link href={`/customer-service-operations/service-requests/${t.id}`} className="font-medium text-gray-900 hover:underline">
                      {t.requestNumber}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-gray-600">{t.subject}</td>
                  <td className="px-4 py-3 text-gray-600">{t.requestType.replace(/_/g, " ")}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <Badge variant={t.status === "completed" ? "success" : t.status === "cancelled" ? "secondary" : "default"}>
