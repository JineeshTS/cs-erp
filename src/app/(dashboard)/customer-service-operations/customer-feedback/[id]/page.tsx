    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500">{record.customerName ?? "Unknown customer"} &mdash; {record.entityType}</p>
        </div>
        {canEdit && (
          <Link
            href={`/customer-service-operations/customer-feedback/${id}/edit`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            { label: "Satisfaction Score", value: record.satisfactionScore?.toString() ?? "-" },
            { label: "Feedback Channel", value: record.feedbackChannel ?? "-" },
          ].map((field) => (
            <div key={field.label}>
        ...
        {record.feedbackText && (
          <div className="mt-6 border-t pt-4">
            <p className="text-xs font-medium text-gray-500">Feedback Text</p>
            <p className="mt-1 whitespace-pre-wrap text-sm text-gray-700">{record.feedbackText}</p>
          </div>
        )}
      </div>
    </div>
