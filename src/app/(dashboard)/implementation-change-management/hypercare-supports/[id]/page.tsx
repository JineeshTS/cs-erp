...
export default async function HypercareSupportDetailPage({
  params,
}: {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500">
            {record.supportType?.replace(/_/g, " ")} &middot; {record.title || "No title"}
          </p>
        </div>
        {canEdit && (
          <div className="flex gap-2">
            <Link
              href={`/implementation-change-management/hypercare-supports/${id}/edit`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div>
            <dt className="text-sm font-medium text-gray-500">Support Ref</dt>
            <dd className="mt-1 text-gray-900">{record.supportRef}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Support Type</dt>
            <dd className="mt-1 text-gray-900 capitalize">{record.supportType?.replace(/_/g, " ")}</dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd className="mt-1 text-gray-900">{record.severity || "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Module</dt>
            <dd className="mt-1 text-gray-900">{record.module || "-"}</dd>
          </div>
          ...
          <div className="sm:col-span-2 lg:col-span-3">
            ...
            <dd className="mt-1 text-gray-900">{record.resolution || "-"}</dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd className="mt-1 text-gray-900">
              {record.resolvedDate ? record.resolvedDate.toLocaleDateString() : "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">SLA Breached</dt>
            <dd className="mt-1 text-gray-900">{record.slaBreached ? "Yes" : "No"}</dd>
          </div>
          <div>
