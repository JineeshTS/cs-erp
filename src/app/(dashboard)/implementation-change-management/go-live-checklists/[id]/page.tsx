...
export default async function GoLiveChecklistDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">{record.checklistRef}</h1>
          <p className="text-sm text-gray-500">{record.checklistType?.replace(/_/g, " ")} &middot; {record.title || "Untitled"}</p>
        </div>
        {canEdit && (
          <Link href={`/implementation-change-management/go-live-checklists/${id}/edit`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div><dt className="text-sm font-medium text-gray-500">Checklist Ref</dt><dd className="mt-1 text-gray-900">{record.checklistRef}</dd></div>
          <div><dt className="text-sm font-medium text-gray-500">Checklist Type</dt><dd className="mt-1 text-gray-900 capitalize">{record.checklistType?.replace(/_/g, " ")}</dd></div>
          ...
          <div><dt className="text-sm font-medium text-gray-500">Total Items</dt><dd className="mt-1 text-gray-900">{record.totalItems ?? "-"}</dd></div>
          <div><dt className="text-sm font-medium text-gray-500">Completed Items</dt><dd className="mt-1 text-gray-900">{record.completedItems ?? "-"}</dd></div>
          <div><dt className="text-sm font-medium text-gray-500">Blocked Items</dt><dd className="mt-1 text-gray-900">{record.blockedItems ?? "-"}</dd></div>
          <div><dt className="text-sm font-medium text-gray-500">Go-Live Date</dt><dd className="mt-1 text-gray-900">{record.goLiveDate ? record.goLiveDate.toLocaleDateString() : "-"}</dd></div>
          ...
          <div><dt className="text-sm font-medium text-gray-500">Approved Date</dt><dd className="mt-1 text-gray-900">{record.approvedDate ? record.approvedDate.toLocaleDateString() : "-"}</dd></div>
          <div><dt className="text-sm font-medium text-gray-500">Is Ready</dt><dd className="mt-1 text-gray-900">{record.isReady ? "Yes" : "No"}</dd></div>
