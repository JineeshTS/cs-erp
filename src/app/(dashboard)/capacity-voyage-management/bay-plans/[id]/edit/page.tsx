...
export default async function EditBayPlanPage({
  params,
}: {
  ...
  const initialData: Record<string, unknown> = {
    vesselScheduleId: bp.vesselScheduleId,
    portRotationId: bp.portRotationId ?? "",
    baplieVersion: bp.baplieVersion ?? "",
    planType: bp.planType,
    totalSlots: Number(bp.totalSlots ?? 0),
    occupiedSlots: Number(bp.occupiedSlots ?? 0),
    utilizationPercent: Number(bp.utilizationPercent ?? 0),
    fileReference: bp.fileReference ?? "",
    status: bp.status,
    notes: bp.notes ?? "",
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/capacity-voyage-management/bay-plans/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">Edit Bay Plan</h1>
      </div>

      <div className="rounded-lg border bg-white p-6">
        <CapForm
          entityType="Bay Plan"
          apiPath={`/api/v1/capacity-voyage-management/bay-plans/${id}`}
          fields={FIELDS}
          initialData={initialData}
          isEdit
          returnPath={`/capacity-voyage-management/bay-plans/${id}`}
        />
      </div>
    </div>
  );
