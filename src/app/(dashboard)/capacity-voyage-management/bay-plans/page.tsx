...
export default async function BayPlansListPage({
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/capacity-voyage-management/bay-plans?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Bay Plans</h1>
            <p className="text-sm text-gray-500">
              Manage BAPLIE bay plan configurations
            </p>
          </div>
        </div>
        {canCreate && (
          <Link
            href="/capacity-voyage-management/bay-plans/new"
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No bay plans found.</p>
          {canCreate && (
            <Link
              href="/capacity-voyage-management/bay-plans/new"
              className="mt-3 inline-block text-sm text-blue-600 hover:underline"
            >
              Create your first bay plan
            </Link>
          )}
        </div>
      ) : (
              ...
              {items.map((bp) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/capacity-voyage-management/bay-plans/${bp.id}`}
