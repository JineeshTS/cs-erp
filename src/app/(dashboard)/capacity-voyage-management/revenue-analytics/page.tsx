...
export default async function RevenueAnalyticsListPage({
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/capacity-voyage-management/revenue-analytics?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Manage revenue per TEU analytics and trade lane performance
            </p>
          </div>
        </div>
        {canCreate && (
          <Link
            href="/capacity-voyage-management/revenue-analytics/new"
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No revenue analytics found.</p>
          {canCreate && (
            <Link
              href="/capacity-voyage-management/revenue-analytics/new"
              className="mt-3 inline-block text-sm text-blue-600 hover:underline"
            >
              Create your first revenue analytics
            </Link>
          )}
        </div>
      ) : (
              ...
              {items.map((ra) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/capacity-voyage-management/revenue-analytics/${ra.id}`}
                  ...
                  <td className="px-4 py-3 text-gray-600">{ra.currency}</td>
                  <td className="px-4 py-3">
                    <Badge
                      variant={
                        ra.status === "published"
