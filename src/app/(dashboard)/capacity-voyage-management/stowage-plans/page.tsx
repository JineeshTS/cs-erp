...
import { Plus, Search, ArrowLeft, Container } from "lucide-react";
...
export default async function StowagePlansListPage({
  ...
  if (search) {
    conditions.push(ilike(capStowagePlans.containerNumber, `%${escapeIlike(search)}%`));
  }
  const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(capStowagePlans.createdAt, capStowagePlans.id, parsedCursor));
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/capacity-voyage-management/stowage-plans?${p.toString()}`;
  }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Manage container stowage positions and plans
            </p>
          </div>
        </div>
        {canCreate && (
          <Link
            href="/capacity-voyage-management/stowage-plans/new"
              ...
              {items.map((sp) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/capacity-voyage-management/stowage-plans/${sp.id}`}
