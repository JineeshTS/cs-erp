import Link from "next/link";
import { Pencil, ArrowLeft } from "lucide-react";
import { getSession } from "@/lib/auth/session";
import { redirect, notFound } from "next/navigation";
import { hasPermission } from "@/lib/rbac";
import { db } from "@/lib/db";
import { eq, and, isNull } from "drizzle-orm";
import { capTransshipmentPlans } from "@/db/schema";
import { Badge } from "@/components/ui/badge";

function fmtDate(d: Date | null): string {
  return d ? new Date(d).toLocaleDateString() : "-";
}

export default async function TransshipmentPlanDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const session = await getSession();
  if (!session) redirect("/login");
  if (!(await hasPermission(session.id, session.tenantId, "capacity:read")))
    redirect("/capacity-voyage-management");

  const { id } = await params;

  const [record] = await db
    .select()
    .from(capTransshipmentPlans)
    .where(
      and(
        eq(capTransshipmentPlans.id, id),
        eq(capTransshipmentPlans.tenantId, session.tenantId),
        isNull(capTransshipmentPlans.deletedAt)
      )
    )
    .limit(1);

  if (!record) notFound();

  const canEdit = await hasPermission(
    session.id,
    session.tenantId,
    "capacity:edit"
  );

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href="/capacity-voyage-management/transshipment-plans"
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">
            Transshipment Plan
          </h1>
          <p className="text-sm text-gray-500">
            {record.originPort} &rarr; {record.transshipmentPort} &rarr;{" "}
            {record.destinationPort}
          </p>
        </div>
        {canEdit && (
          <Link
            href={`/capacity-voyage-management/transshipment-plans/${id}/edit`}
            className="inline-flex items-center gap-1.5 rounded-md border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50"
          >
            <Pencil className="h-3.5 w-3.5" />
            Edit
          </Link>
        )}
      </div>

      <div className="rounded-lg border bg-white p-6">
        <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Booking Reference
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.bookingReference ?? "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Container Number
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.containerNumber ?? "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Origin Port</dt>
            <dd className="mt-1 text-gray-900">{record.originPort}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Transshipment Port
            </dt>
            <dd className="mt-1 text-gray-900">{record.transshipmentPort}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Destination Port
            </dt>
            <dd className="mt-1 text-gray-900">{record.destinationPort}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              First Vessel Schedule ID
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.firstVesselScheduleId ?? "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Second Vessel Schedule ID
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.secondVesselScheduleId ?? "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Expected Arrival
            </dt>
            <dd className="mt-1 text-gray-900">
              {fmtDate(record.expectedArrival)}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Expected Connection
            </dt>
            <dd className="mt-1 text-gray-900">
              {fmtDate(record.expectedConnection)}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Dwell Days</dt>
            <dd className="mt-1 text-gray-900">{record.dwellDays ?? "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Connection Type
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.connectionType ?? "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Status</dt>
            <dd className="mt-1">
              <Badge
                variant={
                  record.status === "completed" ||
                  record.status === "connected"
                    ? "success"
                    : record.status === "failed"
                      ? "destructive"
                      : "secondary"
                }
              >
                {record.status}
              </Badge>
            </dd>
          </div>
          {record.coordinationNotes && (
            <div className="sm:col-span-2 lg:col-span-3">
              <dt className="text-sm font-medium text-gray-500">
                Coordination Notes
              </dt>
              <dd className="mt-1 whitespace-pre-wrap text-gray-900">
                {record.coordinationNotes}
              </dd>
            </div>
          )}
          {record.notes && (
            <div className="sm:col-span-2 lg:col-span-3">
              <dt className="text-sm font-medium text-gray-500">Notes</dt>
              <dd className="mt-1 whitespace-pre-wrap text-gray-900">
                {record.notes}
              </dd>
            </div>
          )}
        </dl>
      </div>
    </div>
  );
}
