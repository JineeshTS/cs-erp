...
export default async function LessonLearnedDetailPage({
  params,
}: {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500">
            {record.lessonType?.replace(/_/g, " ")} &middot; {record.title || "No title"}
          </p>
        </div>
        {canEdit && (
          <div className="flex gap-2">
            <Link
              href={`/knowledge-management-training/lessons-learned/${id}/edit`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div>
            <dt className="text-sm font-medium text-gray-500">Lesson Ref</dt>
            <dd className="mt-1 text-gray-900">{record.lessonRef}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Lesson Type</dt>
            <dd className="mt-1 text-gray-900 capitalize">{record.lessonType?.replace(/_/g, " ")}</dd>
          </div>
          <div>
          ...
          <div className="sm:col-span-2 lg:col-span-3">
            <dt className="text-sm font-medium text-gray-500">Lesson Description</dt>
            <dd className="mt-1 text-gray-900">{record.lessonDescription || "-"}</dd>
          </div>
          <div className="sm:col-span-2 lg:col-span-3">
            <dt className="text-sm font-medium text-gray-500">Recommendation</dt>
            <dd className="mt-1 text-gray-900">{record.recommendation || "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Implemented</dt>
            <dd className="mt-1 text-gray-900">{record.implemented ? "Yes" : "No"}</dd>
          </div>
          <div>
