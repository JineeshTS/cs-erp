...
export default async function SopLibrariesListPage({
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/knowledge-management-training/sop-libraries?${p.toString()}`;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">SOP Libraries</h1>
          <p className="text-sm text-gray-500">Standard operating procedures and process documentation</p>
        </div>
        {canCreate && (
      ...
      <form className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        ...
        {(search || status) && (
          <Link href="/knowledge-management-training/sop-libraries" className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Clear</Link>
        )}
      </form>
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No SOP libraries found.</p>
          {canCreate && (
