...
export default async function CompetencyAssessmentsListPage({
  ...
  const conditions = [
    eq(kmtCompetencyAssessments.tenantId, session.tenantId),
    isNull(kmtCompetencyAssessments.deletedAt),
  ];
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/knowledge-management-training/competency-assessments?${p.toString()}`;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Competency Assessments</h1>
          <p className="text-sm text-gray-500">Employee competency assessment and evaluation records</p>
        </div>
        {canCreate && (
          <Link href="/knowledge-management-training/competency-assessments/new" className="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
            <Plus className="h-4 w-4" /> New Competency Assessment
          </Link>
        )}
      </div>
      ...
      <form className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        <div className="min-w-[200px] flex-1">
          ...
          <div className="relative">
            ...
            <input id="search" name="search" type="text" defaultValue={search} placeholder="Assessment ref, employee name..."
        ...
        {(search || status) && (
          <Link href="/knowledge-management-training/competency-assessments" className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Clear</Link>
        )}
      </form>
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No competency assessments found.</p>
          {canCreate && (
            <Link href="/knowledge-management-training/competency-assessments/new" className="mt-3 inline-block text-sm text-blue-600 hover:underline">Create your first competency assessment</Link>
          )}
        </div>
      ) : (
              ...
              {items.map((t) => (
                <tr key={t.id} className="border-b last:border-0 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link href={`/knowledge-management-training/competency-assessments/${t.id}`} className="font-medium text-gray-900 hover:underline">{t.assessmentRef}</Link>
                  </td>
                  <td className="px-4 py-3 text-gray-600">{t.employeeName || "\u2014"}</td>
                  <td className="px-4 py-3 text-gray-600 capitalize">{t.assessmentType?.replace(/_/g, " ")}</td>
