...
export default async function TrainingModulesListPage({
  ...
  const conditions = [
    eq(kmtTrainingModules.tenantId, session.tenantId),
    isNull(kmtTrainingModules.deletedAt),
  ];
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/knowledge-management-training/training-modules?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Training module creation and management</p>
        </div>
        {canCreate && (
          <Link href="/knowledge-management-training/training-modules/new" className="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
      ...
      <form className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        <div className="min-w-[200px] flex-1">
          ...
          <div className="relative">
            ...
            <input id="search" name="search" type="text" defaultValue={search} placeholder="Module ref, title..."
        ...
        {(search || status) && (
          <Link href="/knowledge-management-training/training-modules" className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Clear</Link>
        )}
      </form>
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No training modules found.</p>
          {canCreate && (
            <Link href="/knowledge-management-training/training-modules/new" className="mt-3 inline-block text-sm text-blue-600 hover:underline">Create your first training module</Link>
          )}
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="px-4 py-3 text-start font-medium text-gray-500">Mandatory</th>
              ...
              {items.map((t) => (
                <tr key={t.id} className="border-b last:border-0 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link href={`/knowledge-management-training/training-modules/${t.id}`} className="font-medium text-gray-900 hover:underline">{t.moduleRef}</Link>
                  </td>
                  <td className="px-4 py-3 text-gray-600">{t.title || "\u2014"}</td>
                  <td className="px-4 py-3 text-gray-600 capitalize">{t.moduleType?.replace(/_/g, " ")}</td>
                  <td className="px-4 py-3 text-gray-600">{t.department || "\u2014"}</td>
                  <td className="px-4 py-3 text-gray-600">{t.durationHours ? `${t.durationHours}h` : "\u2014"}</td>
                  <td className="px-4 py-3 text-gray-600">{t.isMandatory ? "Yes" : "No"}</td>
                  <td className="px-4 py-3">
