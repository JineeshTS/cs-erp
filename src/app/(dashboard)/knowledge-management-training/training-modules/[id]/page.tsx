    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">{record.moduleRef}</h1>
          <p className="text-sm text-gray-500">{record.moduleType?.replace(/_/g, " ")} &middot; {record.title || "Untitled"}</p>
        </div>
        {canEdit && (
          <Link href={`/knowledge-management-training/training-modules/${id}/edit`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div><dt className="text-sm font-medium text-gray-500">Module Ref</dt><dd className="mt-1 text-gray-900">{record.moduleRef}</dd></div>
          <div><dt className="text-sm font-medium text-gray-500">Module Type</dt><dd className="mt-1 text-gray-900 capitalize">{record.moduleType?.replace(/_/g, " ")}</dd></div>
          <div><dt className="text-sm font-medium text-gray-500">Title</dt><dd className="mt-1 text-gray-900">{record.title || "-"}</dd></div>
          <div className="sm:col-span-2 lg:col-span-3"><dt className="text-sm font-medium text-gray-500">Description</dt><dd className="mt-1 text-gray-900">{record.description || "-"}</dd></div>
          ...
          <div><dt className="text-sm font-medium text-gray-500">Duration (Hours)</dt><dd className="mt-1 text-gray-900">{record.durationHours || "-"}</dd></div>
          <div><dt className="text-sm font-medium text-gray-500">Max Participants</dt><dd className="mt-1 text-gray-900">{record.maxParticipants ?? "-"}</dd></div>
          <div><dt className="text-sm font-medium text-gray-500">Passing Score (%)</dt><dd className="mt-1 text-gray-900">{record.passingScorePct || "-"}</dd></div>
          <div><dt className="text-sm font-medium text-gray-500">Mandatory</dt><dd className="mt-1 text-gray-900">{record.isMandatory ? "Yes" : "No"}</dd></div>
          <div><dt className="text-sm font-medium text-gray-500">Validity (Months)</dt><dd className="mt-1 text-gray-900">{record.validityMonths ?? "-"}</dd></div>
