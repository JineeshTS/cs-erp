...
export default async function RegulatoryAlertsListPage({
  ...
  const conditions = [
    eq(kmtRegulatoryAlerts.tenantId, session.tenantId),
    isNull(kmtRegulatoryAlerts.deletedAt),
  ];
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/knowledge-management-training/regulatory-alerts?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Regulatory update alerts and compliance notifications</p>
        </div>
        {canCreate && (
          <Link href="/knowledge-management-training/regulatory-alerts/new" className="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
            <Plus className="h-4 w-4" /> New Regulatory Alert
          </Link>
        )}
      </div>
      ...
      <form className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        <div className="min-w-[200px] flex-1">
          ...
          <div className="relative">
            ...
            <input id="search" name="search" type="text" defaultValue={search} placeholder="Alert ref, title..."
        ...
        {(search || status) && (
          <Link href="/knowledge-management-training/regulatory-alerts" className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Clear</Link>
        )}
      </form>
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No regulatory alerts found.</p>
          {canCreate && (
            <Link href="/knowledge-management-training/regulatory-alerts/new" className="mt-3 inline-block text-sm text-blue-600 hover:underline">Create your first regulatory alert</Link>
          )}
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="px-4 py-3 text-start font-medium text-gray-500">Regulatory Body</th>
              ...
              {items.map((t) => (
                <tr key={t.id} className="border-b last:border-0 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link href={`/knowledge-management-training/regulatory-alerts/${t.id}`} className="font-medium text-gray-900 hover:underline">{t.alertRef}</Link>
                  </td>
                  <td className="px-4 py-3 text-gray-600">{t.title || "\u2014"}</td>
                  <td className="px-4 py-3 text-gray-600 capitalize">{t.alertType?.replace(/_/g, " ")}</td>
                  <td className="px-4 py-3 text-gray-600">{t.regulatoryBody || "\u2014"}</td>
