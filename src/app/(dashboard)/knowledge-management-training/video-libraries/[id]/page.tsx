...
export default async function VideoLibraryDetailPage({
  params,
}: {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500">
            {record.videoType?.replace(/_/g, " ")} &middot; {record.title || "No title"}
          </p>
        </div>
        {canEdit && (
          <div className="flex gap-2">
            <Link
              href={`/knowledge-management-training/video-libraries/${id}/edit`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div>
            <dt className="text-sm font-medium text-gray-500">Video Ref</dt>
            <dd className="mt-1 text-gray-900">{record.videoRef}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Video Type</dt>
            <dd className="mt-1 text-gray-900 capitalize">{record.videoType?.replace(/_/g, " ")}</dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd className="mt-1 text-gray-900">{record.durationMinutes ?? "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Video URL</dt>
            <dd className="mt-1 text-gray-900">{record.videoUrl || "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Thumbnail URL</dt>
            <dd className="mt-1 text-gray-900">{record.thumbnailUrl || "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Language</dt>
            <dd className="mt-1 text-gray-900">{record.language || "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">View Count</dt>
            <dd className="mt-1 text-gray-900">{record.viewCount ?? "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Mandatory</dt>
            <dd className="mt-1 text-gray-900">{record.isMandatory ? "Yes" : "No"}</dd>
          </div>
          <div>
