  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/bunker-fuel-management/reconciliations/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">
          Edit Fuel Reconciliation
        </h1>
      </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        <BfmForm
          ...
          apiPath={`/api/v1/bunker-fuel-management/reconciliations/${id}`}
          ...
          initialData={{
            ...
            openingRob: record.openingRob,
            closingRob: record.closingRob,
            totalReceived: record.totalReceived,
            totalConsumed: record.totalConsumed,
            totalTransferred: record.totalTransferred,
          ...
          isEdit
          returnPath={`/bunker-fuel-management/reconciliations/${id}`}
        />
      </div>
    </div>
  );
