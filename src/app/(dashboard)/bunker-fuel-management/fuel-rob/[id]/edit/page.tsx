...
export default async function EditFuelRobPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/bunker-fuel-management/fuel-rob/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">
          Edit Fuel ROB Record
        </h1>
      </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        <BfmForm
          ...
          apiPath={`/api/v1/bunker-fuel-management/fuel-rob/${id}`}
          ...
          initialData={{
            ...
            robQuantity: record.robQuantity,
            unit: record.unit,
            consumptionDaily: record.consumptionDaily ?? "",
            consumptionVoyage: record.consumptionVoyage ?? "",
          ...
          isEdit
          returnPath={`/bunker-fuel-management/fuel-rob/${id}`}
        />
      </div>
    </div>
  );
