...
export default async function CargoTrackingsListPage({
  ...
  const conditions = [
    eq(thmCargoTrackings.tenantId, session.tenantId),
    isNull(thmCargoTrackings.deletedAt),
  ];
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/transshipment-hub-management/cargo-trackings?${p.toString()}`;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Cargo Trackings</h1>
          <p className="text-sm text-gray-500">Transshipment cargo tracking and movement records</p>
        </div>
        {canCreate && (
          <Link href="/transshipment-hub-management/cargo-trackings/new" className="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
            <Plus className="h-4 w-4" /> New Cargo Tracking
          </Link>
        )}
      </div>
      ...
      <form className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        ...
        {(search || status) && (
          <Link href="/transshipment-hub-management/cargo-trackings" className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Clear</Link>
        )}
      </form>
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No cargo trackings found.</p>
          {canCreate && (
            <Link href="/transshipment-hub-management/cargo-trackings/new" className="mt-3 inline-block text-sm text-blue-600 hover:underline">Create your first cargo tracking</Link>
          )}
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="px-4 py-3 text-start font-medium text-gray-500">Inbound Vessel</th>
              ...
              {items.map((t) => (
                <tr key={t.id} className="border-b last:border-0 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link href={`/transshipment-hub-management/cargo-trackings/${t.id}`} className="font-medium text-gray-900 hover:underline">{t.trackingRef}</Link>
                  </td>
                  ...
                  <td className="px-4 py-3 text-gray-600">{t.inboundVessel || "\u2014"}</td>
                  <td className="px-4 py-3">
