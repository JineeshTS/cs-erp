...
export default async function EditCargoTrackingPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/transshipment-hub-management/cargo-trackings/${id}`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <ThmForm
          ...
          apiPath={`/api/v1/transshipment-hub-management/cargo-trackings/${id}`}
          ...
          initialData={{
            ...
            inboundVessel: record.inboundVessel ?? "",
            outboundVessel: record.outboundVessel ?? "",
            dischargeTime: record.dischargeTime ? record.dischargeTime.toISOString().slice(0, 16) : "",
            loadTime: record.loadTime ? record.loadTime.toISOString().slice(0, 16) : "",
            yardPosition: record.yardPosition ?? "",
            notes: record.notes ?? "",
          }}
          isEdit
          returnPath={`/transshipment-hub-management/cargo-trackings/${id}`}
        />
      </div>
    </div>
  );
