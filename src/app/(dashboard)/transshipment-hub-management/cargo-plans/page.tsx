...
export default async function CargoPlansListPage({
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/transshipment-hub-management/cargo-plans?${p.toString()}`;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Cargo Plans</h1>
          <p className="text-sm text-gray-500">Transshipment hub cargo planning records</p>
        </div>
        {canCreate && (
      ...
      <form className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        ...
        {(search || status) && (
          <Link href="/transshipment-hub-management/cargo-plans" className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Clear</Link>
        )}
      </form>
