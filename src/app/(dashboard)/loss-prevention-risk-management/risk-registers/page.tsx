...
export default async function RiskRegistersListPage({
  ...
  const conditions = [
    eq(lprRiskRegisters.tenantId, session.tenantId),
    isNull(lprRiskRegisters.deletedAt),
  ];
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/loss-prevention-risk-management/risk-registers?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Loss prevention risk register records</p>
        </div>
        {canCreate && (
      ...
      <form className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        <div className="min-w-[200px] flex-1">
          ...
          <div className="relative">
            ...
            <input id="search" name="search" type="text" defaultValue={search} placeholder="Risk ref, title..."
        ...
        {(search || status) && (
          <Link href="/loss-prevention-risk-management/risk-registers" className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Clear</Link>
        )}
      </form>
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          {canCreate && (
            <Link href="/loss-prevention-risk-management/risk-registers/new" className="mt-3 inline-block text-sm text-blue-600 hover:underline">Create your first risk register</Link>
          )}
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="px-4 py-3 text-start font-medium text-gray-500">Risk Type</th>
                <th className="px-4 py-3 text-start font-medium text-gray-500">Risk Score</th>
                <th className="px-4 py-3 text-start font-medium text-gray-500">Risk Owner</th>
              ...
              {items.map((t) => (
                <tr key={t.id} className="border-b last:border-0 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link href={`/loss-prevention-risk-management/risk-registers/${t.id}`} className="font-medium text-gray-900 hover:underline">{t.riskRef}</Link>
                  </td>
