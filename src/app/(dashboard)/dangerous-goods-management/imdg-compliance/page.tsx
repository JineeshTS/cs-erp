...
export default async function ImdgComplianceListPage({
  searchParams,
}: {
  ...
  const { data: items, meta } = await listImdgCompliance({
  ...
  function buildNextUrl(nextCursor: string) {
    ...
    return `/dangerous-goods-management/imdg-compliance?${p.toString()}`;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">IMDG Compliance</h1>
          <p className="text-sm text-gray-500">
            Manage IMDG Code compliance records for dangerous goods classifications
          </p>
        </div>
        {canCreate && (
          <Link
            href="/dangerous-goods-management/imdg-compliance/new"
        ...
        <div className="min-w-[200px] flex-1">
          ...
          <div className="relative">
            ...
            <input
              ...
              type="text"
              defaultValue={search}
              placeholder="Ref, UN number, shipping name..."
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No IMDG compliance records found.</p>
          {canCreate && (
            <Link
              href="/dangerous-goods-management/imdg-compliance/new"
              className="mt-3 inline-block text-sm text-blue-600 hover:underline"
            >
              Create your first IMDG compliance record
            </Link>
          )}
        </div>
      ) : (
              ...
              {items.map((item) => (
                <tr key={item.id} className="border-b last:border-0 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link
                      href={`/dangerous-goods-management/imdg-compliance/${item.id}`}
                  ...
                  <td className="px-4 py-3 text-gray-600">{item.packingGroup || "-"}</td>
                  <td className="px-4 py-3">
                    <Badge
                      variant={
                        item.status === "active"
