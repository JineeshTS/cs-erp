...
export default async function EditPlacardRequirementPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/dangerous-goods-management/placard-requirements/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">
          Edit Placard Requirement
        </h1>
      </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        <DgmForm
          ...
          apiPath={`/api/v1/dangerous-goods-management/placard-requirements/${id}`}
          fields={PLACARD_REQUIREMENT_FIELDS}
          initialData={{
            unNumber: placard.unNumber ?? "",
            imdgClass: placard.imdgClass ?? "",
            subsidiaryRisk: placard.subsidiaryRisk ?? "",
            placardType: placard.placardType ?? "",
            labelCode: placard.labelCode ?? "",
            labelDescription: placard.labelDescription ?? "",
            placementPosition: placard.placementPosition ?? "",
            sizeRequirements: placard.sizeRequirements ?? "",
            colorSpecification: placard.colorSpecification ?? "",
            symbolDescription: placard.symbolDescription ?? "",
            applicableToContainer: placard.applicableToContainer ?? false,
            applicableToVehicle: placard.applicableToVehicle ?? false,
            applicableToPackage: placard.applicableToPackage ?? false,
            marinePollutantMark: placard.marinePollutantMark ?? false,
            elevatedTemperature: placard.elevatedTemperature ?? false,
            fumigationWarning: placard.fumigationWarning ?? false,
            orientationArrows: placard.orientationArrows ?? false,
            imdgReference: placard.imdgReference ?? "",
            imageUrl: placard.imageUrl ?? "",
            notes: placard.notes ?? "",
          }}
          isEdit
          returnPath={`/dangerous-goods-management/placard-requirements/${id}`}
        />
      </div>
    </div>
  );
