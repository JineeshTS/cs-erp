  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/dangerous-goods-management/manifests/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">Edit DG Manifest</h1>
      </div>

      <div className="rounded-lg border bg-white p-6">
        <DgmForm
          entityType="Manifest"
          apiPath={`/api/v1/dangerous-goods-management/manifests/${id}`}
          ...
          initialData={{
            ...
            departureDate: record.departureDate ? new Date(record.departureDate).toISOString() : "",
            arrivalDate: record.arrivalDate ? new Date(record.arrivalDate).toISOString() : "",
            totalDgContainers: record.totalDgContainers ?? "",
            totalDgWeight: record.totalDgWeight ?? "",
            weightUnit: record.weightUnit ?? "",
            submittedToAuthority: record.submittedToAuthority ?? "",
            submissionDate: record.submissionDate ? new Date(record.submissionDate).toISOString() : "",
            submissionReference: record.submissionReference ?? "",
            preparedByName: record.preparedByName ?? "",
            approvedByName: record.approvedByName ?? "",
            approvedAt: record.approvedAt ? new Date(record.approvedAt).toISOString() : "",
            notes: record.notes ?? "",
          }}
          isEdit
          returnPath={`/dangerous-goods-management/manifests/${id}`}
        />
      </div>
    </div>
  );
