...
export default async function EditAssetRegistryPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/fixed-assets-management/asset-registries/${id}`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <FamForm
          ...
          apiPath={`/api/v1/fixed-assets-management/asset-registries/${id}`}
          ...
          initialData={{
            ...
            acquisitionDate: record.acquisitionDate
              ? new Date(record.acquisitionDate).toISOString()
              : "",
            ...
            warrantyExpiry: record.warrantyExpiry
              ? new Date(record.warrantyExpiry).toISOString()
              : "",
            condition: record.condition ?? "",
            notes: record.notes ?? "",
          }}
          isEdit
          returnPath={`/fixed-assets-management/asset-registries/${id}`}
        />
      </div>
    </div>
  );
