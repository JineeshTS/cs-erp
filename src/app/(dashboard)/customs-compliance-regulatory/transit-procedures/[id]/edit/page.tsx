...
export default async function EditTransitProcedurePage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/customs-compliance-regulatory/transit-procedures/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">
          Edit Transit Procedure
        </h1>
      </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        <CcrForm
          ...
          apiPath={`/api/v1/customs-compliance-regulatory/transit-procedures/${id}`}
          fields={TRANSIT_PROCEDURE_FIELDS}
          initialData={{
            procedureType: record.procedureType ?? "",
            declarationNumber: record.declarationNumber ?? "",
            customsOfficeOrigin: record.customsOfficeOrigin ?? "",
            customsOfficeDestination: record.customsOfficeDestination ?? "",
            principalName: record.principalName ?? "",
            principalCode: record.principalCode ?? "",
            guaranteeType: record.guaranteeType ?? "",
            guaranteeAmount: record.guaranteeAmount ?? "",
            guaranteeCurrency: record.guaranteeCurrency ?? "",
            guaranteeReference: record.guaranteeReference ?? "",
            ...
            transitStartAt: record.transitStartAt?.toISOString() ?? "",
            transitDeadlineAt: record.transitDeadlineAt?.toISOString() ?? "",
            transitCompletedAt: record.transitCompletedAt?.toISOString() ?? "",
            discharged: record.discharged ?? false,
            notes: record.notes ?? "",
          }}
          isEdit
          returnPath={`/customs-compliance-regulatory/transit-procedures/${id}`}
        />
      </div>
    </div>
  );
