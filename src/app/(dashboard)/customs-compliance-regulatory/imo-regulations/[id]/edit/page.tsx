...
export default async function EditImoRegulationPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/customs-compliance-regulatory/imo-regulations/${id}`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <CcrForm
          ...
          apiPath={`/api/v1/customs-compliance-regulatory/imo-regulations/${id}`}
          fields={FIELDS}
          initialData={{
            regulationType: record.regulationType ?? "",
            imoReference: record.imoReference ?? "",
            title: record.title ?? "",
            issuingBody: record.issuingBody ?? "",
            conventionName: record.conventionName ?? "",
            publishedAt: record.publishedAt?.toISOString() ?? "",
            effectiveAt: record.effectiveAt?.toISOString() ?? "",
            complianceDeadline: record.complianceDeadline?.toISOString() ?? "",
          ...
          isEdit
          returnPath={`/customs-compliance-regulatory/imo-regulations/${id}`}
        />
      </div>
    </div>
  );
