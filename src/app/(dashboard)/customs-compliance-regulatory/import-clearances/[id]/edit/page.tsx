...
const IMPORT_CLEARANCE_FIELDS: FieldConfig[] = [
  ...
  { name: "invoiceCurrency", label: "Invoice Currency", type: "text", placeholder: "USD" },
...
export default async function EditImportClearancePage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/customs-compliance-regulatory/import-clearances/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">
          Edit Import Clearance
        </h1>
      </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        <CcrForm
          ...
          apiPath={`/api/v1/customs-compliance-regulatory/import-clearances/${id}`}
          ...
          initialData={{
            ...
            importerCode: record.importerCode ?? "",
            importerTaxId: record.importerTaxId ?? "",
            consignmentRef: record.consignmentRef ?? "",
            ...
            invoiceValue: record.invoiceValue != null ? String(record.invoiceValue) : "",
            invoiceCurrency: record.invoiceCurrency ?? "",
            customsValue: record.customsValue != null ? String(record.customsValue) : "",
            dutyAmount: record.dutyAmount != null ? String(record.dutyAmount) : "",
            vatAmount: record.vatAmount != null ? String(record.vatAmount) : "",
            totalTaxes: record.totalTaxes != null ? String(record.totalTaxes) : "",
            paymentMethod: record.paymentMethod ?? "",
            filedAt: record.filedAt
              ? new Date(record.filedAt).toISOString()
              : "",
            clearedAt: record.clearedAt
              ? new Date(record.clearedAt).toISOString()
              : "",
            releaseOrderNumber: record.releaseOrderNumber ?? "",
            inspectionRequired: record.inspectionRequired ?? false,
          ...
          isEdit
          returnPath={`/customs-compliance-regulatory/import-clearances/${id}`}
        />
      </div>
    </div>
  );
