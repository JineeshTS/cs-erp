...
export default async function CustomersPage({
  ...
  if (!(await hasPermission(session.id, session.tenantId, "customer:read")))
    redirect("/");
  ...
  if (search) {
    conditions.push(ilike(scmCustomers.companyName, `%${escapeIlike(search)}%`));
  }
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/customer-management/customers?${p.toString()}`;
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage customer records, KYC, and onboarding
          </p>
        </div>
        {canCreate && (
          <Link
            href="/customer-management/customers/new"
              ...
              {items.map((c) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/customer-management/customers/${c.id}`}
                  ...
                  <td className="px-4 py-3 text-gray-600">
                    {c.creditLimitAmount != null
                      ? `${(c.creditCurrency ?? "USD")} ${c.creditLimitAmount.toLocaleString()}`
                      : "-"}
                  </td>
                  <td className="px-4 py-3">
                    <Badge
