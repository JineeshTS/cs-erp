...
export default async function EditInventoryStockControlPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/procurement-supply-chain/inventory-stock-controls/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">
          Edit Inventory Stock Control
        </h1>
      </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        <PscForm
          ...
          apiPath={`/api/v1/procurement-supply-chain/inventory-stock-controls/${id}`}
          ...
          isEdit
          returnPath={`/procurement-supply-chain/inventory-stock-controls/${id}`}
        />
      </div>
    </div>
  );
