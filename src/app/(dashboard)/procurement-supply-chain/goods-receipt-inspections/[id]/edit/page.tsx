...
export default async function EditGoodsReceiptInspectionPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/procurement-supply-chain/goods-receipt-inspections/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">
          Edit Goods Receipt Inspection
        </h1>
      </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        <PscForm
          ...
          apiPath={`/api/v1/procurement-supply-chain/goods-receipt-inspections/${id}`}
          fields={RECEIPT_FIELDS}
          initialData={{
            receiptType: record.receiptType,
            linkedPoRef: record.linkedPoRef ?? "",
            vendorName: record.vendorName ?? "",
            vendorId: record.vendorId ?? "",
            receiptDate: record.receiptDate
              ? new Date(record.receiptDate).toISOString()
              : "",
            receivedBy: record.receivedBy ?? "",
            totalOrderedQty: record.totalOrderedQty ?? "",
            totalReceivedQty: record.totalReceivedQty ?? "",
            totalAcceptedQty: record.totalAcceptedQty ?? "",
            totalRejectedQty: record.totalRejectedQty ?? "",
            inspectionDate: record.inspectionDate
              ? new Date(record.inspectionDate).toISOString()
              : "",
            inspectedBy: record.inspectedBy ?? "",
            inspectionResult: record.inspectionResult ?? "",
            qualityCertificateRef: record.qualityCertificateRef ?? "",
            warehouseLocation: record.warehouseLocation ?? "",
            deliveryNoteRef: record.deliveryNoteRef ?? "",
            notes: record.notes ?? "",
          }}
          isEdit
          returnPath={`/procurement-supply-chain/goods-receipt-inspections/${id}`}
        />
      </div>
    </div>
  );
