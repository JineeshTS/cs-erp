...
export default async function EditPaymentPredictionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Edit Payment Prediction</h1>
        <p className="text-sm text-gray-500">Update prediction {record.predictionRef}</p>
      </div>
      <ArccForm
        ...
        initialData={record}
        isEdit
        apiPath={`/api/v1/accounts-receivable-credit-control/payment-predictions/${id}`}
