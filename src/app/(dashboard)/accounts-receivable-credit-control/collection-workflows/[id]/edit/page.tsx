...
export default async function EditCollectionWorkflowPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Edit Collection Workflow</h1>
        <p className="text-sm text-gray-500">Update workflow for {workflow.customerName}</p>
      </div>
      <ArccForm
        entityType="Collection Workflow"
        fields={fields}
        initialData={workflow}
        apiPath={`/api/v1/accounts-receivable-credit-control/collection-workflows/${id}`}
        method="PUT"
