...
export default async function EditBadDebtProvisionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Edit Bad Debt Provision</h1>
        <p className="text-sm text-gray-500">Update provision for {provision.customerName}</p>
      </div>
      <ArccForm
        ...
        isEdit
        apiPath={`/api/v1/accounts-receivable-credit-control/bad-debt-provisions/${id}`}
