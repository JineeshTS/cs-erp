...
export default async function MaximizationEngineDetailPage({
  params,
}: {
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500">
            {record.engineType?.replace(/_/g, " ")} &middot; {record.tradeLane || "No trade lane"}
          </p>
        </div>
        {canEdit && (
          <div className="flex gap-2">
            <Link
              href={`/liner-revenue-management/maximization-engines/${id}/edit`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div>
            ...
            <dd className="mt-1 text-gray-900">{record.modelName || "-"}</dd>
          </div>
          <div>
          ...
          <div>
            <dt className="text-sm font-medium text-gray-500">Recommended Rate</dt>
            <dd className="mt-1 text-gray-900">{record.recommendedRate ?? "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Current Rate</dt>
            <dd className="mt-1 text-gray-900">{record.currentRate ?? "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Uplift %</dt>
            <dd className="mt-1 text-gray-900">{record.upliftPct ?? "-"}</dd>
          </div>
          <div>
          ...
          <div>
            <dt className="text-sm font-medium text-gray-500">Simulation Run At</dt>
            <dd className="mt-1 text-gray-900">
              {record.simulationRunAt ? record.simulationRunAt.toLocaleDateString() : "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Accepted Recommendation</dt>
            <dd className="mt-1 text-gray-900">{record.acceptedRecommendation ? "Yes" : "No"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Revenue Impact</dt>
            <dd className="mt-1 text-gray-900">{record.revenueImpact ?? "-"}</dd>
          </div>
          <div>
