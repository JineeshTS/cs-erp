...
export default async function TeuMaximizationsListPage({
  searchParams,
}: {
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/liner-revenue-management/teu-maximizations?${p.toString()}`;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">TEU Maximizations</h1>
          <p className="text-sm text-gray-500">Revenue per TEU optimization strategies</p>
        </div>
        {canCreate && (
          <Link href="/liner-revenue-management/teu-maximizations/new" className="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
            <Plus className="h-4 w-4" /> New TEU Maximization
          </Link>
        )}
      </div>
      ...
      <form className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        ...
        {(search || status) && (
          <Link href="/liner-revenue-management/teu-maximizations" className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Clear</Link>
        )}
      </form>
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No TEU maximizations found.</p>
          {canCreate && (
            <Link href="/liner-revenue-management/teu-maximizations/new" className="mt-3 inline-block text-sm text-blue-600 hover:underline">Create your first TEU maximization</Link>
          )}
        </div>
      ) : (
