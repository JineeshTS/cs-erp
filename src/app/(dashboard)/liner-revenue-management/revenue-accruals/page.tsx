...
export default async function RevenueAccrualsListPage({
  ...
  const conditions = [
    eq(lrmRevenueAccruals.tenantId, session.tenantId),
    isNull(lrmRevenueAccruals.deletedAt),
  ];
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/liner-revenue-management/revenue-accruals?${p.toString()}`;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Revenue Accruals</h1>
          <p className="text-sm text-gray-500">Liner revenue accrual records</p>
        </div>
        {canCreate && (
          <Link href="/liner-revenue-management/revenue-accruals/new" className="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
      ...
      <form className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        <div className="min-w-[200px] flex-1">
          ...
          <div className="relative">
            ...
            <input id="search" name="search" type="text" defaultValue={search} placeholder="Accrual ref, voyage ref..."
        ...
        {(search || status) && (
          <Link href="/liner-revenue-management/revenue-accruals" className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Clear</Link>
        )}
      </form>
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          {canCreate && (
            <Link href="/liner-revenue-management/revenue-accruals/new" className="mt-3 inline-block text-sm text-blue-600 hover:underline">Create your first revenue accrual</Link>
          )}
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="px-4 py-3 text-start font-medium text-gray-500">Accrual Amt</th>
              ...
              {items.map((t) => (
                <tr key={t.id} className="border-b last:border-0 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link href={`/liner-revenue-management/revenue-accruals/${t.id}`} className="font-medium text-gray-900 hover:underline">{t.accrualRef}</Link>
                  </td>
                  <td className="px-4 py-3 text-gray-600">{t.voyageRef || "\u2014"}</td>
                  <td className="px-4 py-3 text-gray-600 capitalize">{t.accrualType?.replace(/_/g, " ")}</td>
                  <td className="px-4 py-3 text-gray-600">{t.accrualAmount ?? "\u2014"}</td>
                  <td className="px-4 py-3 text-gray-600">{t.varianceAmount ?? "\u2014"}</td>
                  <td className="px-4 py-3">
