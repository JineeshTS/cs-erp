...
export default async function EditRegulatoryComplianceCalendarPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/audit-compliance-management/regulatory-compliance-calendars/${id}`}
        ...
        <h1 className="text-2xl font-bold text-gray-900">
          Edit Regulatory Compliance Calendar Entry
        </h1>
      </div>
      <div className="rounded-lg border bg-white p-6">
        <AcmForm
          entityType="Regulatory Compliance Calendar"
          apiPath={`/api/v1/audit-compliance-management/regulatory-compliance-calendars/${id}`}
          fields={fields}
          initialData={record as Record<string, unknown>}
          isEdit
          returnPath={`/audit-compliance-management/regulatory-compliance-calendars/${id}`}
        />
      </div>
    </div>
  );
