...
import { listInternalAudits } from "@/lib/audit-compliance-management/service";
...
export default async function InternalAuditsPage({
  ...
  const { data: items, meta } = await listInternalAudits({
  ...
  function buildUrl(params: Record<string, string>) {
    ...
    return `/audit-compliance-management/internal-audits?${p.toString()}`;
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage and track internal audit engagements
          </p>
        </div>
        {canCreate && (
          <Link
            href="/audit-compliance-management/internal-audits/new"
            className="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700"
          >
            <Plus className="h-4 w-4" /> New Internal Audit
          </Link>
        )}
      </div>
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          {canCreate && (
            ...
            >
              Create your first internal audit
            </Link>
          )}
        </div>
      ) : (
              ...
              {items.map((t) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/audit-compliance-management/internal-audits/${t.id}`}
