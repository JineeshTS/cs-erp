...
export default async function CampaignsPage({
  ...
  if (search) {
    conditions.push(ilike(scmCampaigns.campaignName, `%${search}%`));
  }
  if (cursor)
    conditions.push(lt(scmCampaigns.createdAt, new Date(cursor)));
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/sales-crm/campaigns?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No campaigns found.</p>
          {canCreate && (
            <Link
              href="/sales-crm/campaigns/new"
              ...
              {items.map((campaign) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/sales-crm/campaigns/${campaign.id}`}
