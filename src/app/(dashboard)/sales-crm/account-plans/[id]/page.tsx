...
export default async function AccountPlanDetailPage({
  params,
}: {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          {canEdit && (
            <Link
              href={`/sales-crm/account-plans/${id}/edit`}
          ...
          {canDelete && (
            <DeleteButton apiPath={`/api/v1/sales-crm/account-plans/${id}`} />
          )}
        </div>
      </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            { label: "Plan Name", value: record.planName },
            { label: "Fiscal Year", value: record.fiscalYear.toString() },
            { label: "Account Manager ID", value: record.accountManagerId },
            { label: "Revenue Target", value: record.revenueTargetAmount?.toLocaleString() ?? "-" },
            { label: "TEU Target", value: record.teuTarget?.toLocaleString() ?? "-" },
            { label: "Retention Strategy", value: record.retentionStrategy ?? "-" },
            { label: "Growth Strategy", value: record.growthStrategy ?? "-" },
            { label: "Risk Assessment", value: record.riskAssessment ?? "-" },
            { label: "Competitive Analysis", value: record.competitiveAnalysis ?? "-" },
            { label: "Review Date", value: record.reviewDate ? new Date(record.reviewDate).toLocaleDateString() : "-" },
