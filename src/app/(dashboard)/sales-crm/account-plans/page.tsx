...
export default async function AccountPlansPage({
  ...
  if (search) {
    conditions.push(ilike(scmAccountPlans.planName, `%${search}%`));
  }
  if (cursor)
    conditions.push(lt(scmAccountPlans.createdAt, new Date(cursor)));
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/sales-crm/account-plans?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage customer account plans and strategies
          </p>
        </div>
        {canCreate && (
          <Link
            href="/sales-crm/account-plans/new"
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No account plans found.</p>
          {canCreate && (
            <Link
              href="/sales-crm/account-plans/new"
              className="mt-3 inline-block text-sm text-blue-600 hover:underline"
            >
              Add your first account plan
            </Link>
          )}
        </div>
      ) : (
              ...
              {items.map((ap) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/sales-crm/account-plans/${ap.id}`}
                  ...
                  <td className="px-4 py-3 text-gray-600">
                    {ap.revenueTargetAmount?.toLocaleString() ?? "-"}
                  </td>
                  <td className="px-4 py-3 text-gray-600">
                    {ap.teuTarget?.toLocaleString() ?? "-"}
                  </td>
                  <td className="px-4 py-3">
                    <Badge
                      variant={
