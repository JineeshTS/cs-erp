...
export default async function ContractLineItemsPage({
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/sales-crm/contract-line-items?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage charge items for contracts
          </p>
        </div>
        {canCreate && (
          <Link
            href={`/sales-crm/contract-line-items/new${contractId ? `?contractId=${contractId}` : ""}`}
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No contract line items found.</p>
          {canCreate && (
            <Link
              href="/sales-crm/contract-line-items/new"
              ...
              {items.map((li) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/sales-crm/contract-line-items/${li.id}`}
