...
export default async function CustomerSegmentsPage({
  ...
  if (search) {
    conditions.push(ilike(scmCustomerSegments.segmentName, `%${escapeIlike(search)}%`));
  }
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/sales-crm/customer-segments?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage customer segmentation categories
          </p>
        </div>
        {canCreate && (
          <Link
            href="/sales-crm/customer-segments/new"
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No segments found.</p>
          {canCreate && (
            <Link
              href="/sales-crm/customer-segments/new"
              ...
              {items.map((s) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/sales-crm/customer-segments/${s.id}`}
