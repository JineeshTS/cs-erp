...
export default async function SalesTargetsPage({
  ...
  if (search) {
    conditions.push(ilike(scmSalesTargets.targetName, `%${escapeIlike(search)}%`));
  }
  const parsedCursor = parseCompoundCursor(cursor);
    if (parsedCursor) conditions.push(cursorCondition(scmSalesTargets.createdAt, scmSalesTargets.id, parsedCursor));
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/sales-crm/sales-targets?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage sales performance targets and actuals
          </p>
        </div>
        {canCreate && (
          <Link
            href="/sales-crm/sales-targets/new"
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No sales targets found.</p>
          {canCreate && (
            <Link
              href="/sales-crm/sales-targets/new"
              className="mt-3 inline-block text-sm text-blue-600 hover:underline"
            >
              Add your first sales target
            </Link>
          )}
        </div>
      ) : (
              ...
              {items.map((st) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/sales-crm/sales-targets/${st.id}`}
                  ...
                  <td className="px-4 py-3 text-gray-600">
                    {st.targetType.replace(/_/g, " ")}
                  </td>
                  <td className="px-4 py-3 text-gray-600">
                    {st.fiscalYear}
                  </td>
                  <td className="px-4 py-3 text-gray-600">
                    {st.revenueTarget?.toLocaleString() ?? "-"}
                  </td>
                  <td className="px-4 py-3 text-gray-600">
                    {st.revenueActual?.toLocaleString() ?? "-"}
                  </td>
                  <td className="px-4 py-3">
                    <Badge
                      variant={
                        st.status === "achieved"
                          ? "default"
                          : st.status === "missed" || st.status === "cancelled"
                            ? "destructive"
                            : st.status === "active"
                              ? "secondary"
                              : "outline"
