  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/port-agency-management/port-call-plans/${id}`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <PamForm
          ...
          apiPath={`/api/v1/port-agency-management/port-call-plans/${id}`}
          fields={PORT_CALL_PLAN_FIELDS}
          initialData={{
            planType: plan.planType,
            vesselName: plan.vesselName,
            imoNumber: plan.imoNumber ?? "",
            voyageRef: plan.voyageRef ?? "",
            portName: plan.portName,
            portCode: plan.portCode ?? "",
            berthName: plan.berthName ?? "",
            terminalName: plan.terminalName ?? "",
            agentName: plan.agentName ?? "",
            agentContactEmail: plan.agentContactEmail ?? "",
            agentContactPhone: plan.agentContactPhone ?? "",
            eta: plan.eta ? new Date(plan.eta).toISOString() : "",
            etd: plan.etd ? new Date(plan.etd).toISOString() : "",
            pilotRequired: plan.pilotRequired ?? false,
            tugRequired: plan.tugRequired ?? false,
            tugsCount: plan.tugsCount ?? "",
            specialInstructions: plan.specialInstructions ?? "",
            portChargesEstimate: plan.portChargesEstimate ?? "",
            portChargesCurrency: plan.portChargesCurrency ?? "",
            notes: plan.notes ?? "",
          }}
          isEdit
          returnPath={`/port-agency-management/port-call-plans/${id}`}
        />
      </div>
    </div>
  );
