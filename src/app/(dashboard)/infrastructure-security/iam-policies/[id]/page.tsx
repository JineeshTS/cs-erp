...
export default async function IamPolicyDetailPage({
  params,
}: {
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500">
            {record.policyCode} &middot; {record.policyType}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {canEdit && (
            <Link
              href={`/infrastructure-security/iam-policies/${id}/edit`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            { label: "Policy Type", value: record.policyType },
            { label: "Effect", value: record.effect },
            { label: "Priority", value: record.priority?.toString() ?? "-" },
          ...
          <div>
            <p className="text-xs font-medium text-gray-500">Effect</p>
            <div className="mt-0.5">
              <Badge
                variant={
                  record.effect === "allow" ? "success" : "destructive"
                }
              >
                {record.effect}
              </Badge>
