...
import { ArrowLeft, Plus, Search, Key } from "lucide-react";
...
export default async function ApiKeysListPage({
  searchParams,
}: {
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/infrastructure-security/api-keys?${p.toString()}`;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {canCreate && (
          <Link
            href="/infrastructure-security/api-keys/new"
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No API keys found.</p>
          {canCreate && (
            <Link
              href="/infrastructure-security/api-keys/new"
              ...
              {items.map((k) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/infrastructure-security/api-keys/${k.id}`}
