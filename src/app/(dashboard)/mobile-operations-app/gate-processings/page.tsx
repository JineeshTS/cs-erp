...
export default async function GateProcessingsListPage({
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/mobile-operations-app/gate-processings?${p.toString()}`;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Gate Processings</h1>
          <p className="text-sm text-gray-500">Mobile gate in/out processing records</p>
        </div>
        {canCreate && (
      ...
      <form className="flex flex-wrap items-end gap-3 rounded-lg border bg-white p-4">
        <div className="min-w-[200px] flex-1">
          ...
          <div className="relative">
            ...
            <input id="search" name="search" type="text" defaultValue={search} placeholder="Gate ref, container number..."
        ...
        {(search || status) && (
          <Link href="/mobile-operations-app/gate-processings" className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Clear</Link>
        )}
      </form>
