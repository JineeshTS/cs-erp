...
export default async function GateProcessingDetailPage({
  params,
}: {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500">
            {record.gateType?.replace(/_/g, " ")} &middot; {record.containerNumber || "No container"}
          </p>
        </div>
        {canEdit && (
          <div className="flex gap-2">
            <Link
              href={`/mobile-operations-app/gate-processings/${id}/edit`}
      ...
      <div className="rounded-lg border bg-white p-6">
        <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div>
            <dt className="text-sm font-medium text-gray-500">Gate Ref</dt>
            <dd className="mt-1 text-gray-900">{record.gateRef}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Gate Type</dt>
            <dd className="mt-1 text-gray-900 capitalize">{record.gateType?.replace(/_/g, " ")}</dd>
          </div>
          <div>
          ...
          <div>
            <dt className="text-sm font-medium text-gray-500">Truck Plate</dt>
            <dd className="mt-1 text-gray-900">{record.truckPlate || "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Driver Name</dt>
            <dd className="mt-1 text-gray-900">{record.driverName || "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Driver License</dt>
            <dd className="mt-1 text-gray-900">{record.driverLicense || "-"}</dd>
          </div>
          <div>
          ...
          <div>
            <dt className="text-sm font-medium text-gray-500">Gate Number</dt>
            <dd className="mt-1 text-gray-900">{record.gateNumber || "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Processed At</dt>
            <dd className="mt-1 text-gray-900">
              {record.processedAt ? record.processedAt.toLocaleString() : "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Yard Location</dt>
            <dd className="mt-1 text-gray-900">{record.yardLocation || "-"}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Damage Found</dt>
            <dd className="mt-1 text-gray-900">{record.damageFound ? "Yes" : "No"}</dd>
          </div>
          <div>
