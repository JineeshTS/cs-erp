...
export default async function EditColdChainDocPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/reefer-container-management/cold-chain-docs/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">
          Edit Cold Chain Document
        </h1>
      </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        <RcmForm
          ...
          apiPath={`/api/v1/reefer-container-management/cold-chain-docs/${id}`}
          ...
          initialData={{
            ...
            phytosanitaryCert: record.phytosanitaryCert ?? "",
            healthCert: record.healthCert ?? "",
            fumigationCert: record.fumigationCert ?? "",
            temperatureLogUrl: record.temperatureLogUrl ?? "",
          ...
          isEdit
          returnPath={`/reefer-container-management/cold-chain-docs/${id}`}
        />
      </div>
    </div>
  );
