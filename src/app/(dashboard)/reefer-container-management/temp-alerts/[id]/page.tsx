import Link from "next/link";
import { Pencil, ArrowLeft } from "lucide-react";
import { getSession } from "@/lib/auth/session";
import { redirect, notFound } from "next/navigation";
import { hasPermission } from "@/lib/rbac";
import { getTempAlert } from "@/lib/reefer-container-management/service";
import { Badge } from "@/components/ui/badge";

const severityVariant = {
  warning: "warning",
  critical: "destructive",
  emergency: "destructive",
} as const;

export default async function TempAlertDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const session = await getSession();
  if (!session) redirect("/login");
  if (!(await hasPermission(session.id, session.tenantId, "reefer:read")))
    redirect("/reefer-container-management");

  const { id } = await params;

  const record = await getTempAlert(id, session.tenantId);
  if (!record) notFound();

  const canEdit = await hasPermission(
    session.id,
    session.tenantId,
    "reefer:edit"
  );

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href="/reefer-container-management/temp-alerts"
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">
            {record.alertRef}
          </h1>
          <p className="text-sm text-gray-500">
            Container {record.containerNumber} &middot; {record.alertType}
          </p>
        </div>
        {canEdit && (
          <div className="flex gap-2">
            <Link
              href={`/reefer-container-management/temp-alerts/${id}/edit`}
              className="inline-flex items-center gap-1.5 rounded-md border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50"
            >
              <Pencil className="h-3.5 w-3.5" />
              Edit
            </Link>
          </div>
        )}
      </div>

      <div className="rounded-lg border bg-white p-6">
        <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div>
            <dt className="text-sm font-medium text-gray-500">Alert Ref</dt>
            <dd className="mt-1 text-gray-900">{record.alertRef}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Container</dt>
            <dd className="mt-1 text-gray-900">{record.containerNumber}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Booking Ref</dt>
            <dd className="mt-1 text-gray-900">
              {record.bookingRef || "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Alert Type</dt>
            <dd className="mt-1 text-gray-900">{record.alertType}</dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Alert Severity
            </dt>
            <dd className="mt-1">
              <Badge
                variant={
                  severityVariant[
                    record.alertSeverity as keyof typeof severityVariant
                  ] ?? "secondary"
                }
              >
                {record.alertSeverity}
              </Badge>
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Set Point Temp (&deg;C)
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.setPointTempC ?? "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Actual Temp (&deg;C)
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.actualTempC ?? "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Deviation (&deg;C)
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.deviationC ?? "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Threshold (&deg;C)
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.thresholdC ?? "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Exceedance Duration (min)
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.exceedanceDurationMinutes ?? "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Triggered At</dt>
            <dd className="mt-1 text-gray-900">
              {record.triggeredAt
                ? new Date(record.triggeredAt).toLocaleString()
                : "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Acknowledged At
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.acknowledgedAt
                ? new Date(record.acknowledgedAt).toLocaleString()
                : "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Acknowledged By
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.acknowledgedByName || "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Escalation Level
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.escalationLevel ?? "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Escalated To
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.escalatedToName || "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Escalated At
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.escalatedAt
                ? new Date(record.escalatedAt).toLocaleString()
                : "-"}
            </dd>
          </div>
          <div className="sm:col-span-2 lg:col-span-3">
            <dt className="text-sm font-medium text-gray-500">
              Correction Action
            </dt>
            <dd className="mt-1 whitespace-pre-wrap text-gray-900">
              {record.correctionAction || "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Resolved At</dt>
            <dd className="mt-1 text-gray-900">
              {record.resolvedAt
                ? new Date(record.resolvedAt).toLocaleString()
                : "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Resolved By</dt>
            <dd className="mt-1 text-gray-900">
              {record.resolvedByName || "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">
              Cargo Impact
            </dt>
            <dd className="mt-1 text-gray-900">
              {record.cargoImpact || "-"}
            </dd>
          </div>
          <div>
            <dt className="text-sm font-medium text-gray-500">Status</dt>
            <dd className="mt-1">
              <Badge
                variant={
                  record.status === "active"
                    ? "warning"
                    : record.status === "resolved" ||
                        record.status === "closed"
                      ? "success"
                      : record.status === "escalated"
                        ? "destructive"
                        : "secondary"
                }
              >
                {record.status}
              </Badge>
            </dd>
          </div>
          <div className="sm:col-span-2 lg:col-span-3">
            <dt className="text-sm font-medium text-gray-500">Notes</dt>
            <dd className="mt-1 whitespace-pre-wrap text-gray-900">
              {record.notes || "-"}
            </dd>
          </div>
        </dl>
      </div>
    </div>
  );
}
