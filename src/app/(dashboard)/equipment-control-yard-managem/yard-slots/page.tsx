...
export default async function YardSlotsListPage({
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/equipment-control-yard-managem/yard-slots?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Yard Slots</h1>
            <p className="text-sm text-gray-500">
              Manage yard planning and slot allocation
            </p>
          </div>
        </div>
        {canCreate && (
          <Link
            href="/equipment-control-yard-managem/yard-slots/new"
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No yard slots found.</p>
          {canCreate && (
            <Link
              href="/equipment-control-yard-managem/yard-slots/new"
              className="mt-3 inline-block text-sm text-blue-600 hover:underline"
            >
              Create your first yard slot
            </Link>
          )}
        </div>
      ) : (
              ...
              {items.map((ys) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/equipment-control-yard-managem/yard-slots/${ys.id}`}
                  ...
                  <td className="px-4 py-3 text-gray-600">
                    {ys.currentOccupancy ?? 0} / {ys.slotCapacity ?? 0}
                  </td>
                  <td className="px-4 py-3">
                    <Badge
                      variant={
