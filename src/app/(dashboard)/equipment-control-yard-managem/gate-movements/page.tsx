...
export default async function GateMovementsListPage({
  ...
  function buildNextUrl(nextCur: string) {
    ...
    return `/equipment-control-yard-managem/gate-movements?${p.toString()}`;
  }
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Manage gate in/out CODECO movements
            </p>
          </div>
        </div>
        {canCreate && (
          <Link
            href="/equipment-control-yard-managem/gate-movements/new"
      ...
      {items.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No gate movements found.</p>
          {canCreate && (
            <Link
              href="/equipment-control-yard-managem/gate-movements/new"
              className="mt-3 inline-block text-sm text-blue-600 hover:underline"
            >
              Create your first gate movement
            </Link>
          )}
        </div>
      ) : (
              ...
              {items.map((gm) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/equipment-control-yard-managem/gate-movements/${gm.id}`}
                      className="font-medium text-gray-900 hover:underline"
                    >
                      {gm.movementReference}
                    </Link>
                  </td>
