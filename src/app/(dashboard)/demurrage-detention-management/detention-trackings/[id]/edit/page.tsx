...
export default async function EditDetentionTrackingPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/demurrage-detention-management/detention-trackings/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">
          Edit Detention Tracking
        </h1>
      </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        <DdmForm
          ...
          apiPath={`/api/v1/demurrage-detention-management/detention-trackings/${id}`}
          fields={DETENTION_TRACKING_FIELDS}
          initialData={{
            containerNumber: tracking.containerNumber ?? "",
            containerSize: tracking.containerSize ?? "",
            containerType: tracking.containerType ?? "",
            bookingRef: tracking.bookingRef ?? "",
            blNumber: tracking.blNumber ?? "",
            customerName: tracking.customerName ?? "",
            gateOutDate: tracking.gateOutDate
              ? new Date(tracking.gateOutDate).toISOString()
              : "",
            gateInDate: tracking.gateInDate
              ? new Date(tracking.gateInDate).toISOString()
              : "",
            freeTimeDays: tracking.freeTimeDays ?? "",
            freeTimeExpiry: tracking.freeTimeExpiry
              ? new Date(tracking.freeTimeExpiry).toISOString()
              : "",
            detentionDays: tracking.detentionDays ?? "",
            dailyRate: tracking.dailyRate ?? "",
            totalAmount: tracking.totalAmount ?? "",
            currency: tracking.currency ?? "",
            depotName: tracking.depotName ?? "",
            depotLocation: tracking.depotLocation ?? "",
            containerCondition: tracking.containerCondition ?? "",
            damageNotes: tracking.damageNotes ?? "",
            notes: tracking.notes ?? "",
          }}
          isEdit
          returnPath={`/demurrage-detention-management/detention-trackings/${id}`}
        />
      </div>
    </div>
  );
