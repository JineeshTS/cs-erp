...
export default async function BlContainerDetailPage({
  params,
}: {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">{record.containerNumber}</h1>
          <p className="text-sm text-gray-500">BL Container Details</p>
        </div>
        <div className="flex items-center gap-2">
          {canEdit && (
            <Link href={`/operations-documentation/bl-containers/${id}/edit`} className="inline-flex items-center gap-1.5 rounded-md border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50">
              <Pencil className="h-3.5 w-3.5" /> Edit
            </Link>
          )}
          {canDelete && (
            <form action={`/api/v1/operations-documentation/bl-containers/${id}`} method="POST">
      ...
      <div className="rounded-lg border bg-white p-6">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            { label: "Gross Weight", value: record.grossWeight?.toString() ?? "-" },
            { label: "Tare Weight", value: record.tareWeight?.toString() ?? "-" },
            { label: "Net Weight", value: record.netWeight?.toString() ?? "-" },
            { label: "Volume (CBM)", value: record.volumeCbm?.toString() ?? "-" },
            { label: "Package Count", value: record.packageCount?.toString() ?? "-" },
