  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/intermodal-icd-operations/dry-ports/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">Edit Dry Port</h1>
      </div>

      <div className="rounded-lg border bg-white p-6">
        <IcdForm
          entityType="Dry Port"
          apiPath={`/api/v1/intermodal-icd-operations/dry-ports/${id}`}
          ...
          initialData={{
            ...
            customsZoneType: record.customsZoneType ?? "",
            storageCapacityTeu: record.storageCapacityTeu ?? "",
            currentOccupancyTeu: record.currentOccupancyTeu ?? "",
            railConnected: record.railConnected ?? false,
            gateHoursStart: record.gateHoursStart ?? "",
            gateHoursEnd: record.gateHoursEnd ?? "",
          ...
          isEdit
          returnPath={`/intermodal-icd-operations/dry-ports/${id}`}
        />
      </div>
    </div>
  );
