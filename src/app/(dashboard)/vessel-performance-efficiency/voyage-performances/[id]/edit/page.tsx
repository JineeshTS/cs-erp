  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/vessel-performance-efficiency/voyage-performances/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">
          Edit Voyage Performance
        </h1>
      </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        <VpeForm
          ...
          apiPath={`/api/v1/vessel-performance-efficiency/voyage-performances/${id}`}
          ...
          initialData={{
            ...
            charterPartyId: record.charterPartyId ?? "",
            cpSpeed: record.cpSpeed ?? "",
            actualSpeed: record.actualSpeed ?? "",
            speedVariance: record.speedVariance ?? "",
            cpConsumption: record.cpConsumption ?? "",
            actualConsumption: record.actualConsumption ?? "",
            consumptionVariance: record.consumptionVariance ?? "",
            goodWeatherDays: record.goodWeatherDays ?? "",
            badWeatherDays: record.badWeatherDays ?? "",
            claimAmount: record.claimAmount ?? "",
            claimCurrency: record.claimCurrency ?? "",
            claimDirection: record.claimDirection ?? "",
            notes: record.notes ?? "",
          }}
          isEdit
          returnPath={`/vessel-performance-efficiency/voyage-performances/${id}`}
        />
      </div>
    </div>
  );
