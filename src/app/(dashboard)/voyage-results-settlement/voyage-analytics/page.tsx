  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Voyage Analytics</h1>
          <p className="text-sm text-gray-500">Manage voyage analytics and reports</p>
        </div>
        {canCreate && (
          <Link
            href="/voyage-results-settlement/voyage-analytics/new"
        ...
        <div className="overflow-hidden rounded-lg border bg-white">
              ...
              {result.data.map((record) => (
                <tr key={record.id} className="border-b last:border-0 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link
                      href={`/voyage-results-settlement/voyage-analytics/${record.id}`}
          ...
          {result.meta.hasMore && result.meta.cursor && (
            <div className="border-t px-4 py-3 text-center">
              <Link
                href={`/voyage-results-settlement/voyage-analytics?cursor=${result.meta.cursor}${sp.search ? `&search=${sp.search}` : ""}${sp.status ? `&status=${sp.status}` : ""}`}
