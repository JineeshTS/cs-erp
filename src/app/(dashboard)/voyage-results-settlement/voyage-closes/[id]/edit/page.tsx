  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link
          href={`/voyage-results-settlement/voyage-closes/${id}`}
          className="rounded-md border p-2 hover:bg-gray-50"
        >
          <ArrowLeft className="h-4 w-4" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">Edit Voyage Close</h1>
      </div>
      <div className="rounded-lg border bg-white p-6">
        <VrsForm
          entityType="Voyage Close"
          apiPath={`/api/v1/voyage-results-settlement/voyage-closes/${id}`}
          fields={fields}
          initialData={record as unknown as Record<string, unknown>}
          isEdit
          returnPath={`/voyage-results-settlement/voyage-closes/${id}`}
        />
      </div>
    </div>
  );
