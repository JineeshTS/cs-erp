...
export default async function TcSettlementsListPage({
  searchParams,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">TC Settlements</h1>
          <p className="text-sm text-gray-500">Manage time charter settlement records</p>
        </div>
        {canCreate && (
          <Link
            href="/voyage-results-settlement/tc-settlements/new"
      ...
      {result.data.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          ...
          <p className="mt-3 text-gray-500">No TC settlements found.</p>
        </div>
      ) : (
        <div className="overflow-hidden rounded-lg border bg-white">
              ...
              {result.data.map((record) => (
                <tr key={record.id} className="border-b last:border-0 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link
                      href={`/voyage-results-settlement/tc-settlements/${record.id}`}
          ...
          {result.meta.hasMore && result.meta.cursor && (
            <div className="border-t px-4 py-3 text-center">
              <Link
                href={`/voyage-results-settlement/tc-settlements?cursor=${result.meta.cursor}${sp.search ? `&search=${sp.search}` : ""}${sp.status ? `&status=${sp.status}` : ""}`}
