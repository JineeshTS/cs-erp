...
export default async function EditPortCostPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/port-disbursement-accounting/port-costs/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">Edit Port Cost</h1>
      </div>

      <div className="rounded-lg border bg-white p-6">
        <PdaForm
          entityType="Port Cost"
          apiPath={`/api/v1/port-disbursement-accounting/port-costs/${id}`}
          ...
          initialData={{
            ...
            maximumCharge: record.maximumCharge ?? "",
            ...
            effectiveTo: record.effectiveTo
              ...
              : "",
            vesselSizeFrom: record.vesselSizeFrom ?? "",
            vesselSizeTo: record.vesselSizeTo ?? "",
            cargoTypeApplicable: record.cargoTypeApplicable ?? "",
            sourceDocument: record.sourceDocument ?? "",
            notes: record.notes ?? "",
          }}
          isEdit
          returnPath={`/port-disbursement-accounting/port-costs/${id}`}
        />
      </div>
    </div>
  );
