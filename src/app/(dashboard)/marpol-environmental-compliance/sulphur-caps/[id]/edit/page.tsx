...
export default async function EditSulphurCapPage({
  params,
}: {
  ...
  const initialData: Record<string, unknown> = {
    sulphurType: record.sulphurType,
    title: record.title,
    vesselName: record.vesselName,
    imoNumber: record.imoNumber,
    fuelType: record.fuelType,
    sulphurContent: record.sulphurContent,
    sampleDate: record.sampleDate,
    labReference: record.labReference,
    hasScrubber: record.hasScrubber,
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <Link
          href={`/marpol-environmental-compliance/sulphur-caps/${id}`}
          className="text-sm text-muted-foreground hover:underline"
        >
          &larr; Back to Detail
        </Link>
        <h1 className="text-2xl font-bold">Edit Sulphur Cap</h1>
      </div>
      ...
      <MecForm
        ...
        isEdit
        returnPath={`/marpol-environmental-compliance/sulphur-caps/${id}`}
      />
    </div>
  );
}
