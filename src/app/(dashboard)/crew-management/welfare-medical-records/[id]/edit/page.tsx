...
export default async function EditWelfareMedicalRecordPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link
          href={`/crew-management/welfare-medical-records/${record.id}`}
          className="text-sm text-muted-foreground hover:underline"
        >
          &larr; Back to {record.recordRef}
        </Link>
      ...
      <CrmForm
        ...
        apiPath={`/api/v1/crew-management/welfare-medical-records/${record.id}`}
        fields={fields}
        initialData={record as unknown as Record<string, unknown>}
        isEdit
        returnPath={`/crew-management/welfare-medical-records/${record.id}`}
        method="PATCH"
      />
    </div>
  );
