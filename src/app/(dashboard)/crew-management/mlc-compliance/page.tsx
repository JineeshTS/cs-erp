...
import { listMlcCompliance } from "@/lib/crew-management/service";
...
export default async function MlcCompliancePage({
  searchParams,
}: {
  ...
  const { data, meta } = await listMlcCompliance({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">MLC Compliance</h1>
        <Link
          href="/crew-management/mlc-compliance/new"
      ...
      {data.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed p-12 text-center">
          <p className="text-muted-foreground">No MLC compliance records found.</p>
          <Link
            href="/crew-management/mlc-compliance/new"
            className="mt-4 text-sm text-primary underline"
          >
            Create your first MLC compliance record
          </Link>
        </div>
      ) : (
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="px-4 py-3 text-start font-medium">Flag State</th>
                  <th className="px-4 py-3 text-start font-medium">MLC Standard</th>
                ...
                {data.map((row) => (
                  <tr key={row.id} className="border-b hover:bg-muted/30">
                    <td className="px-4 py-3">
                      <Link
                        href={`/crew-management/mlc-compliance/${row.id}`}
                    ...
                    <td className="px-4 py-3">{row.mlcStandard}</td>
                    <td className="px-4 py-3">{row.complianceArea}</td>
          ...
          <div className="flex items-center justify-between">
            ...
            {meta?.cursor && (
              <Link
                href={`/crew-management/mlc-compliance?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(meta.cursor)}`}
