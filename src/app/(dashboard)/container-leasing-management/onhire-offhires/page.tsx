...
export default async function OnhireOffhiresPage({
  searchParams,
}: {
  ...
  const result = await listOnhireOffhires({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage container on-hire and off-hire events
          </p>
        </div>
        {canCreate && (
          <Link
            href="/container-leasing-management/onhire-offhires/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by event ref or container number..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-end font-medium">Days</th>
              ...
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      No on-hire / off-hire events found
                    </p>
                  </div>
                </td>
              </tr>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/container-leasing-management/onhire-offhires/${row.id}`}
                  ...
                  <td className="px-4 py-3">{row.eventLocation}</td>
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/container-leasing-management/onhire-offhires?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
