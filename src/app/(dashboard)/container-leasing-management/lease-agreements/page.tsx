...
import { listLeaseAgreements } from "@/lib/container-leasing-management/service";
...
export default async function LeaseAgreementsPage({
  ...
  const result = await listLeaseAgreements({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage container lease agreements
          </p>
        </div>
        {canCreate && (
          <Link
            href="/container-leasing-management/lease-agreements/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by agreement ref or lessor name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-end font-medium">Daily Rate</th>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/container-leasing-management/lease-agreements/${row.id}`}
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/container-leasing-management/lease-agreements?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
