...
export default async function ContainerRedeliveriesPage({
  searchParams,
}: {
  ...
  const result = await listContainerRedeliveries({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">
            Container Redeliveries
          </h1>
          <p className="text-sm text-muted-foreground">
            Manage container redelivery records
          </p>
        </div>
        {canCreate && (
          <Link
            href="/container-leasing-management/container-redeliveries/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by redelivery ref or container number..."
              ...
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      No container redeliveries found
                    </p>
                  </div>
                </td>
              </tr>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/container-leasing-management/container-redeliveries/${row.id}`}
                      className="font-medium text-primary hover:underline"
                    >
                      {row.redeliveryRef}
                    </Link>
                  </td>
                  <td className="px-4 py-3">{row.containerNumber}</td>
                  <td className="px-4 py-3 capitalize">
                    {row.redeliveryType?.replace(/_/g, " ")}
                  </td>
                  <td className="px-4 py-3">
                    {row.redeliveryLocation ?? "\u2014"}
                  </td>
                  <td className="px-4 py-3">
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/container-leasing-management/container-redeliveries?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
