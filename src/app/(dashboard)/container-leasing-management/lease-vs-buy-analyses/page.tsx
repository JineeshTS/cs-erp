...
export default async function LeaseVsBuyAnalysesPage({
  searchParams,
}: {
  ...
  const result = await listLeaseVsBuyAnalyses({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage lease vs buy financial analyses
          </p>
        </div>
        {canCreate && (
          <Link
            href="/container-leasing-management/lease-vs-buy-analyses/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by analysis ref or container type..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-end font-medium">NPV Lease</th>
              <th className="px-4 py-3 text-end font-medium">NPV Buy</th>
              ...
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      No lease vs buy analyses found
                    </p>
                  </div>
                </td>
              </tr>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/container-leasing-management/lease-vs-buy-analyses/${row.id}`}
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/container-leasing-management/lease-vs-buy-analyses?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
