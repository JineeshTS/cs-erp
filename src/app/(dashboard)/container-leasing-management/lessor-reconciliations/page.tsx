...
export default async function LessorReconciliationsPage({
  searchParams,
}: {
  ...
  const result = await listLessorReconciliations({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">
            Lessor Reconciliations
          </h1>
          <p className="text-sm text-muted-foreground">
            Manage lessor reconciliation records
          </p>
        </div>
        {canCreate && (
          <Link
            href="/container-leasing-management/lessor-reconciliations/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by reconciliation ref or lessor name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-start font-medium">Lessor</th>
              <th className="px-4 py-3 text-start font-medium">Type</th>
              <th className="px-4 py-3 text-end font-medium">Difference</th>
              <th className="px-4 py-3 text-end font-medium">Unmatched</th>
              ...
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      No lessor reconciliations found
                    </p>
                  </div>
                </td>
              </tr>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/container-leasing-management/lessor-reconciliations/${row.id}`}
                      className="font-medium text-primary hover:underline"
                    >
                      {row.reconciliationRef}
                    </Link>
                  </td>
                  <td className="px-4 py-3">{row.lessorName}</td>
                  <td className="px-4 py-3 capitalize">
                    {row.reconciliationType?.replace(/_/g, " ")}
                  </td>
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/container-leasing-management/lessor-reconciliations?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
