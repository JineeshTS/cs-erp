...
import { listGaAgreements } from "@/lib/agent-network-management/service";
...
export default async function GaAgreementsListPage({
  searchParams,
}: {
  ...
  const { data, meta } = await listGaAgreements({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">GA Agreements</h1>
        {canCreate && (
          <Link
            href="/agent-network-management/ga-agreements/new"
      ...
      <form method="GET" className="flex items-center gap-4">
        <input
          type="text"
          name="search"
          placeholder="Search by agreement ref or agent..."
          defaultValue={search}
      ...
      {data.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed p-12 text-center">
          <p className="text-muted-foreground">No GA agreements found</p>
          {canCreate && (
            <Link
              href="/agent-network-management/ga-agreements/new"
              className="mt-4 text-sm text-primary underline"
            >
              Create your first GA agreement
            </Link>
          )}
        </div>
      ) : (
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="px-4 py-3 text-start font-medium">Expiry</th>
                ...
                {data.map((row) => (
                  <tr key={row.id} className="border-b hover:bg-muted/30">
                    <td className="px-4 py-3">
                      <Link
                        href={`/agent-network-management/ga-agreements/${row.id}`}
                    ...
                    <td className="px-4 py-3 capitalize">{row.agreementType?.replace(/_/g, " ")}</td>
          ...
          <div className="flex items-center justify-between">
            ...
            {meta?.cursor && (
              <Link
                href={`/agent-network-management/ga-agreements?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(meta.cursor)}`}
