...
export default async function EditGaAgreementPage({
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link
          href={`/agent-network-management/ga-agreements/${record.id}`}
      ...
      <AnmForm
        ...
        apiPath={`/api/v1/agent-network-management/ga-agreements/${record.id}`}
        fields={fields}
        initialData={{
          ...
          portsCovered: record.portsCovered ?? "",
          commencementDate: record.commencementDate ? new Date(record.commencementDate).toISOString().slice(0, 16) : "",
          expiryDate: record.expiryDate ? new Date(record.expiryDate).toISOString().slice(0, 16) : "",
          autoRenewal: record.autoRenewal ? "true" : "",
          terminationNoticeDays: record.terminationNoticeDays ?? "",
          baseCommissionPct: record.baseCommissionPct ?? "",
          commissionCurrency: record.commissionCurrency ?? "",
          exclusivityClause: record.exclusivityClause ? "true" : "",
          performanceGuarantee: record.performanceGuarantee ?? "",
          notes: record.notes ?? "",
        }}
        isEdit
        returnPath={`/agent-network-management/ga-agreements/${record.id}`}
      />
    </div>
  );
}
