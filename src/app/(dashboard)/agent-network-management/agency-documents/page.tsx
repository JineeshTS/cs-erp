...
import { listAgencyDocuments } from "@/lib/agent-network-management/service";
...
export default async function AgencyDocumentsPage({
  ...
  const result = await listAgencyDocuments({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage agency agreement documents and amendments
          </p>
        </div>
        {canCreate && (
          <Link
            href="/agent-network-management/agency-documents/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by document ref or agent..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-start font-medium">Effective</th>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/agent-network-management/agency-documents/${row.id}`}
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/agent-network-management/agency-documents?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
