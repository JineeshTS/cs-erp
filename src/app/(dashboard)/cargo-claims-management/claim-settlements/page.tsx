...
import { listClaimSettlements } from "@/lib/cargo-claims-management/service";
import { Badge } from "@/components/ui/badge";

export default async function ClaimSettlementsPage({
  searchParams,
}: {
  ...
  const result = await listClaimSettlements({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage claim settlement and payment processing
          </p>
        </div>
        {canCreate && (
          <Link
            href="/cargo-claims-management/claim-settlements/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by settlement ref or vessel name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-end font-medium">Settled Amt</th>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/cargo-claims-management/claim-settlements/${row.id}`}
                  ...
                  <td className="px-4 py-3 capitalize">
                    {row.settlementType?.replace(/_/g, " ")}
                  </td>
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/cargo-claims-management/claim-settlements?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
