...
import { listDamageSurveys } from "@/lib/cargo-claims-management/service";
import { Badge } from "@/components/ui/badge";

export default async function DamageSurveysPage({
  searchParams,
}: {
  ...
  const result = await listDamageSurveys({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage cargo damage survey documentation
          </p>
        </div>
        {canCreate && (
          <Link
            href="/cargo-claims-management/damage-surveys/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by survey ref or vessel name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-start font-medium">Surveyor</th>
              <th className="px-4 py-3 text-end font-medium">Damage Est.</th>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/cargo-claims-management/damage-surveys/${row.id}`}
                  ...
                  <td className="px-4 py-3 capitalize">
                    {row.surveyType?.replace(/_/g, " ")}
                  </td>
                  <td className="px-4 py-3">{row.surveyorName ?? "—"}</td>
                  <td className="px-4 py-3 text-end">
                    {row.estimatedDamageUsd ?? "—"}
                  </td>
                  <td className="px-4 py-3">
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/cargo-claims-management/damage-surveys?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
