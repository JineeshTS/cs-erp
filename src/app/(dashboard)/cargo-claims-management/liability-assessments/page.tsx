...
export default async function LiabilityAssessmentsPage({
  searchParams,
}: {
  ...
  const result = await listLiabilityAssessments({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage liability assessments for cargo claims
          </p>
        </div>
        {canCreate && (
          <Link
            href="/cargo-claims-management/liability-assessments/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by assessment ref or vessel name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-start font-medium">Convention</th>
              <th className="px-4 py-3 text-start font-medium">Recommended</th>
              ...
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      No liability assessments found
                    </p>
                  </div>
                </td>
              </tr>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/cargo-claims-management/liability-assessments/${row.id}`}
                  ...
                  <td className="px-4 py-3 capitalize">
                    {row.assessmentType?.replace(/_/g, " ")}
                  </td>
                  <td className="px-4 py-3">
                    {row.applicableConvention ?? "—"}
                  </td>
                  <td className="px-4 py-3">
                    {row.recommendedLiability ?? "—"}
                  </td>
                  <td className="px-4 py-3">
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/cargo-claims-management/liability-assessments?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
