...
export default async function EditLiabilityAssessmentPage({
  ...
  const initialData: Record<string, string> = {
    ...
    applicableConvention: record.applicableConvention ?? "",
    liabilityBasis: record.liabilityBasis ?? "",
    limitationApplied: record.limitationApplied ? "true" : "",
    limitPerPackage: record.limitPerPackage ?? "",
    limitPerKg: record.limitPerKg ?? "",
    totalPackages: record.totalPackages?.toString() ?? "",
    totalWeightKg: record.totalWeightKg ?? "",
    calculatedLimit: record.calculatedLimit ?? "",
    defenseApplicable: record.defenseApplicable ?? "",
    defenseDescription: record.defenseDescription ?? "",
    recommendedLiability: record.recommendedLiability ?? "",
    carrierLiabilityPct: record.carrierLiabilityPct ?? "",
    notes: record.notes ?? "",
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link
          href={`/cargo-claims-management/liability-assessments/${id}`}
        ...
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">
            Edit {record.assessmentRef}
          </h1>
          <p className="text-sm text-muted-foreground">
            Update liability assessment details
          </p>
        </div>
      </div>
      ...
      <div className="rounded-md border p-6">
        <CcmForm
          ...
          apiPath={`/api/v1/cargo-claims-management/liability-assessments/${id}`}
