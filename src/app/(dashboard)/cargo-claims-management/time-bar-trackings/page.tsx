...
import { listTimeBarTrackings } from "@/lib/cargo-claims-management/service";
import { Badge } from "@/components/ui/badge";

export default async function TimeBarTrackingsPage({
  searchParams,
}: {
  ...
  const result = await listTimeBarTrackings({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage time bar tracking and deadline alerts
          </p>
        </div>
        {canCreate && (
          <Link
            href="/cargo-claims-management/time-bar-trackings/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by tracking ref or vessel name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-end font-medium">Days Left</th>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/cargo-claims-management/time-bar-trackings/${row.id}`}
                  ...
                  <td className="px-4 py-3 capitalize">
                    {row.trackingType?.replace(/_/g, " ")}
                  </td>
                  <td className="px-4 py-3">
                    {row.timeBarDeadline
                      ? new Date(row.timeBarDeadline).toLocaleDateString()
                      : "—"}
                  </td>
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/cargo-claims-management/time-bar-trackings?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
