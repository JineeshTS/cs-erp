...
export default async function PortfolioAnalyticsPage({
  ...
  const result = await listPortfolioAnalytics({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage claims portfolio analytics and reporting
          </p>
        </div>
        {canCreate && (
          <Link
            href="/cargo-claims-management/portfolio-analytics/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by analytics ref or top claim category..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-end font-medium">Total Claims</th>
              <th className="px-4 py-3 text-end font-medium">Loss Ratio</th>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/cargo-claims-management/portfolio-analytics/${row.id}`}
                  ...
                  <td className="px-4 py-3 capitalize">
                    {row.analyticsType?.replace(/_/g, " ") ?? "—"}
                  </td>
                  <td className="px-4 py-3 text-end">
                    {row.totalClaimsCount ?? "—"}
                  </td>
                  <td className="px-4 py-3 text-end">
                    {row.lossRatio ?? "—"}
                  </td>
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/cargo-claims-management/portfolio-analytics?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
