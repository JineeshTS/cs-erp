...
export default async function EditBiReportPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/analytics-business-intelligence/bi-reports/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">Edit BI Report</h1>
      </div>

      <div className="rounded-lg border bg-white p-6">
        <AbiForm
          entityType="BI Report"
          apiPath={`/api/v1/analytics-business-intelligence/bi-reports/${id}`}
          ...
          isEdit
          returnPath={`/analytics-business-intelligence/bi-reports/${id}`}
        />
      </div>
    </div>
  );
