...
export default async function OcrResultsListPage() {
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              View OCR processing results for scanned documents
            </p>
          </div>
        </div>
      </div>
      ...
      {data.length === 0 ? (
        <div className="rounded-lg border bg-white px-8 py-12 text-center">
          <ScanSearch className="mx-auto h-10 w-10 text-gray-300" />
          <p className="mt-3 text-gray-500">
            No OCR results found. OCR processing will appear here when documents
            are analyzed.
          </p>
        </div>
      ) : (
              ...
              {data.map((result) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/document-management-system/ocr-results/${result.id}`}
