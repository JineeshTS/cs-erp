...
import { listHireSurveys } from "@/lib/survey-inspection-management/service";
...
export default async function HireSurveysListPage({
  searchParams,
}: {
  ...
  const { data, meta } = await listHireSurveys({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Hire Surveys</h1>
            <p className="text-sm text-muted-foreground">
              Manage on-hire and off-hire vessel surveys
            </p>
          </div>
        </div>
        {canCreate && (
          <Link
            href="/survey-inspection-management/hire-surveys/new"
      ...
      <form method="get" className="flex flex-wrap items-center gap-3">
        <input
          name="search"
          type="text"
          placeholder="Ref, vessel, charterer..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-start font-medium">Charterer</th>
            ...
            {data.map((record) => (
              <tr key={record.id} className="border-b last:border-0 hover:bg-muted/30">
                <td className="px-4 py-3">
                  <Link
                    href={`/survey-inspection-management/hire-surveys/${record.id}`}
                ...
                <td className="px-4 py-3">{record.chartererName}</td>
      ...
      <div className="flex items-center justify-end gap-3">
        {meta.hasMore && meta.cursor && (
          <Link
            href={{
              pathname: "/survey-inspection-management/hire-surveys",
              query: {
                cursor: meta.cursor,
