...
export default async function EditHatchInspectionPage({
  params,
}: {
  ...
  const initialData: Record<string, string | number | boolean> = {
    ...
    holdNumber: record.holdNumber ?? "",
    hatchCoverType: record.hatchCoverType ?? "",
    cleanliness: record.cleanliness ?? "",
    dryness: record.dryness ?? "",
    odorFree: record.odorFree ?? false,
    previousCargo: record.previousCargo ?? "",
    residueFound: record.residueFound ?? false,
    residueDescription: record.residueDescription ?? "",
    hatchCoverSeal: record.hatchCoverSeal ?? "",
    waterTightness: record.waterTightness ?? "",
    ventilationOk: record.ventilationOk ?? false,
    bilgesClean: record.bilgesClean ?? false,
    ladderCondition: record.ladderCondition ?? "",
    lightingOk: record.lightingOk ?? false,
    cargoFitness: record.cargoFitness ?? "",
    inspectorName: record.inspectorName ?? "",
    inspectorCompany: record.inspectorCompany ?? "",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Edit {record.inspectionRef}</h1>
          <p className="text-sm text-muted-foreground">
            Update hatch inspection details
          </p>
        </div>
      </div>
      ...
      <SimForm
        ...
        apiPath={`/api/v1/survey-inspection-management/hatch-inspections/${id}`}
        fields={fields}
        initialData={initialData}
        isEdit
        returnPath={`/survey-inspection-management/hatch-inspections/${id}`}
      />
    </div>
  );
}
