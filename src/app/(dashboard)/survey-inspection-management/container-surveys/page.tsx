...
import { listContainerSurveys } from "@/lib/survey-inspection-management/service";
...
export default async function ContainerSurveysListPage({
  ...
  const { data, meta } = await listContainerSurveys({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Container Surveys</h1>
            <p className="text-sm text-muted-foreground">
              Manage container condition surveys and MNR
            </p>
          </div>
        </div>
        {(await hasPermission(session.id, session.tenantId, "survey:create")) && (
          <Link
            href="/survey-inspection-management/container-surveys/new"
            ...
            {data.map((record) => (
              <tr key={record.id} className="border-b last:border-0 hover:bg-muted/30">
                <td className="px-4 py-3">
                  <Link
                    href={`/survey-inspection-management/container-surveys/${record.id}`}
                ...
                <td className="px-4 py-3">{record.overallCondition}</td>
                <td className="px-4 py-3">
      ...
      <div className="flex items-center justify-end gap-3">
        {meta.hasMore && meta.cursor && (
          <Link
            href={{
              pathname: "/survey-inspection-management/container-surveys",
              query: {
                cursor: meta.cursor,
