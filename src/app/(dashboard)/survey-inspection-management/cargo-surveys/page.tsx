...
import { listCargoSurveys } from "@/lib/survey-inspection-management/service";
...
export default async function CargoSurveysListPage({
  searchParams,
}: {
  ...
  const { data, meta } = await listCargoSurveys({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Cargo Surveys</h1>
            <p className="text-sm text-muted-foreground">
              Manage pre-load cargo survey operations
            </p>
          </div>
        </div>
        {(await hasPermission(session.id, session.tenantId, "survey:create")) && (
          <Link
            href="/survey-inspection-management/cargo-surveys/new"
            ...
            {data.map((record) => (
              <tr key={record.id} className="border-b last:border-0 hover:bg-muted/30">
                <td className="px-4 py-3">
                  <Link
                    href={`/survey-inspection-management/cargo-surveys/${record.id}`}
      ...
      <div className="flex items-center justify-end gap-3">
        {meta.hasMore && meta.cursor && (
          <Link
            href={{
              pathname: "/survey-inspection-management/cargo-surveys",
              query: {
                cursor: meta.cursor,
