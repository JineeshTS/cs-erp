...
export default async function EditTariffRatePage({
  params,
}: {
  ...
  const initialData: Record<string, unknown> = {
    ...
    minimumCharge: item.minimumCharge != null ? Number(item.minimumCharge) : "",
    maximumCharge: item.maximumCharge != null ? Number(item.maximumCharge) : "",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/commercial-pricing-management/tariff-rates/${id}`}
          className="rounded-md p-2 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5" />
        </Link>
        <h1 className="text-2xl font-bold">Edit Tariff Rate</h1>
      </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        <CpmForm
          ...
          fields={fields}
          apiPath={`/api/v1/commercial-pricing-management/tariff-rates/${id}`}
