...
export default async function SlotSwapCoordinationsPage({
  searchParams,
}: {
  ...
  const result = await listSlotSwapCoordinations({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">
            Slot Swap Coordinations
          </h1>
          <p className="text-sm text-muted-foreground">
            Manage slot swap agreements with partner lines
          </p>
        </div>
        {canCreate && (
          <Link
            href="/liner-operations-control/slot-swap-coordinations/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by swap ref or partner name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-start font-medium">Partner</th>
              <th className="px-4 py-3 text-start font-medium">Type</th>
              <th className="px-4 py-3 text-start font-medium">Vessel</th>
              <th className="px-4 py-3 text-end font-medium">Total Value</th>
              ...
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      No slot swap coordinations found
                    </p>
                  </div>
                </td>
              </tr>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/liner-operations-control/slot-swap-coordinations/${row.id}`}
                  ...
                  <td className="px-4 py-3 capitalize">
                    {row.swapType?.replace(/_/g, " ")}
                  </td>
                  <td className="px-4 py-3">{row.vesselName ?? "\u2014"}</td>
                  <td className="px-4 py-3 text-end">
                    {row.totalValue ?? "\u2014"} {row.swapCurrency ?? ""}
                  </td>
                  <td className="px-4 py-3">
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/liner-operations-control/slot-swap-coordinations?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
