...
export default async function LoadFactorReportsPage({
  searchParams,
}: {
  ...
  const result = await listLoadFactorReports({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage load factor and utilization reporting
          </p>
        </div>
        {canCreate && (
          <Link
            href="/liner-operations-control/load-factor-reports/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by report ref or vessel name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-end font-medium">Load Factor %</th>
              <th className="px-4 py-3 text-end font-medium">Revenue/TEU</th>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/liner-operations-control/load-factor-reports/${row.id}`}
                  ...
                  <td className="px-4 py-3 text-end">
                    {row.loadFactorPercentage ?? "\u2014"}
                  </td>
                  <td className="px-4 py-3 text-end">
                    {row.revenuePerTeu ?? "\u2014"} {row.reportCurrency ?? ""}
                  </td>
                  <td className="px-4 py-3">
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/liner-operations-control/load-factor-reports?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
