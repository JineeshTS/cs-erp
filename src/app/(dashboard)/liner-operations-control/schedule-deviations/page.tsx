...
export default async function ScheduleDeviationsPage({
  searchParams,
}: {
  ...
  const result = await listScheduleDeviations({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage schedule deviations and recovery plans
          </p>
        </div>
        {canCreate && (
          <Link
            href="/liner-operations-control/schedule-deviations/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by deviation ref or vessel name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-end font-medium">Delay Hrs</th>
              <th className="px-4 py-3 text-end font-medium">Cost Impact</th>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/liner-operations-control/schedule-deviations/${row.id}`}
                  ...
                  <td className="px-4 py-3 capitalize">
                    {row.deviationType?.replace(/_/g, " ")}
                  </td>
                  <td className="px-4 py-3 text-end">
                    {row.delayHours ?? "\u2014"}
                  </td>
                  <td className="px-4 py-3 text-end">
                    {row.costImpact ?? "\u2014"} {row.impactCurrency ?? ""}
                  </td>
                  <td className="px-4 py-3">
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/liner-operations-control/schedule-deviations?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
