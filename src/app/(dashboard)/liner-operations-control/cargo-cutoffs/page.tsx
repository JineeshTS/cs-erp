...
export default async function CargoCutoffsPage({
  searchParams,
}: {
  ...
  const result = await listCargoCutoffs({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage cargo cut-off schedules per port and vessel
          </p>
        </div>
        {canCreate && (
          <Link
            href="/liner-operations-control/cargo-cutoffs/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by cutoff ref or port name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-start font-medium">Cutoff Date</th>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/liner-operations-control/cargo-cutoffs/${row.id}`}
                  ...
                  <td className="px-4 py-3 capitalize">
                    {row.cutoffType?.replace(/_/g, " ")}
                  </td>
                  <td className="px-4 py-3">{row.vesselName ?? "\u2014"}</td>
                  <td className="px-4 py-3">
                    {row.cutoffDatetime
                      ? new Date(row.cutoffDatetime).toLocaleString()
                      : "\u2014"}
                  </td>
                  <td className="px-4 py-3">
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/liner-operations-control/cargo-cutoffs?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
