...
export default async function OverbookingRolloversPage({
  searchParams,
}: {
  ...
  const result = await listOverbookingRollovers({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">
            Overbooking Rollovers
          </h1>
          <p className="text-sm text-muted-foreground">
            Manage overbooking, rollover, and cargo bump records
          </p>
        </div>
        {canCreate && (
          <Link
            href="/liner-operations-control/overbooking-rollovers/new"
      ...
      <form method="GET" className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            name="search"
            defaultValue={search}
            placeholder="Search by rollover ref or vessel name..."
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="px-4 py-3 text-start font-medium">Containers</th>
              ...
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      No overbooking rollovers found
                    </p>
                  </div>
                </td>
              </tr>
              ...
              result.data.map((row) => (
                <tr key={row.id} className="border-b last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      href={`/liner-operations-control/overbooking-rollovers/${row.id}`}
                  ...
                  <td className="px-4 py-3 capitalize">
                    {row.rolloverType?.replace(/_/g, " ")}
                  </td>
                  <td className="px-4 py-3">{row.portName ?? "\u2014"}</td>
                  <td className="px-4 py-3">{row.containerCount ?? "\u2014"}</td>
                  <td className="px-4 py-3">
      ...
      {result.meta?.cursor && (
        <div className="flex justify-center">
          <Link
            href={`/liner-operations-control/overbooking-rollovers?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&cursor=${encodeURIComponent(result.meta.cursor)}`}
