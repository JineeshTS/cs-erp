  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          href={`/master-data-management/ports/${id}`}
          className="rounded-md p-1 hover:bg-gray-100"
        >
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">Edit Port</h1>
      </div>

      <div className="rounded-lg border bg-white p-6">
        <MdmForm
          entityType="Port"
          apiPath={`/api/v1/master-data-management/ports/${id}`}
          fields={PORT_FIELDS}
          initialData={{
            ...
            latitude: port.latitude ? Number(port.latitude) : "",
            longitude: port.longitude ? Number(port.longitude) : "",
            isMajorPort: port.isMajorPort,
          }}
          isEdit
          returnPath={`/master-data-management/ports/${id}`}
        />
      </div>
    </div>
  );
