...
import { listEsgKpis } from "@/lib/sustainability-esg-reporting/service";
...
export default async function EsgKpisListPage({
  searchParams,
}: {
  ...
  const { data, meta } = await listEsgKpis({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold">ESG KPIs</h1>
          <p className="text-sm text-muted-foreground">
            Track environmental, social, and governance key performance
            indicators
          </p>
        </div>
        {canCreate && (
          <Link
            href="/sustainability-esg-reporting/esg-kpis/new"
      ...
      <form method="GET" className="flex items-center gap-4">
        <input
          type="text"
          name="search"
          placeholder="Search by KPI ref or name..."
          defaultValue={search}
      ...
      {data.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed p-12 text-center">
          <p className="text-muted-foreground">No ESG KPIs found.</p>
          {canCreate && (
            <Link
              href="/sustainability-esg-reporting/esg-kpis/new"
              className="mt-4 text-sm text-primary underline"
            >
              Create your first ESG KPI
            </Link>
          )}
        </div>
      ) : (
        <>
                ...
                {data.map((row) => (
                  <tr key={row.id} className="border-b hover:bg-muted/30">
                    <td className="px-4 py-3">
                      <Link
                        href={`/sustainability-esg-reporting/esg-kpis/${row.id}`}
                    ...
                    <td className="px-4 py-3">{row.kpiType}</td>
                    <td className="px-4 py-3">
                      {row.reportingPeriod ?? "\u2014"}
                    </td>
                    <td className="px-4 py-3">
          ...
          <div className="flex items-center justify-between">
            ...
            {meta?.cursor && (
              <Link
                href={`/sustainability-esg-reporting/esg-kpis?search=${encodeURIComponent(search)}&cursor=${encodeURIComponent(meta.cursor)}`}
