...
export async function listProjectPlans({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(icmProjectPlans.createdAt, icmProjectPlans.id, cc)); }
  const results = await db.select().from(icmProjectPlans).where(and(...conditions)).orderBy(desc(icmProjectPlans.createdAt), desc(icmProjectPlans.id)).limit(limit + 1);
...
export async function listDataMigrations({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(icmDataMigrations.createdAt, icmDataMigrations.id, cc)); }
  const results = await db.select().from(icmDataMigrations).where(and(...conditions)).orderBy(desc(icmDataMigrations.createdAt), desc(icmDataMigrations.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listUatManagements({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(icmUatManagements.createdAt, icmUatManagements.id, cc)); }
  const results = await db.select().from(icmUatManagements).where(and(...conditions)).orderBy(desc(icmUatManagements.createdAt), desc(icmUatManagements.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listGoLiveChecklists({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(icmGoLiveChecklists.createdAt, icmGoLiveChecklists.id, cc)); }
  const results = await db.select().from(icmGoLiveChecklists).where(and(...conditions)).orderBy(desc(icmGoLiveChecklists.createdAt), desc(icmGoLiveChecklists.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listChangeRequests({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(icmChangeRequests.createdAt, icmChangeRequests.id, cc)); }
  const results = await db.select().from(icmChangeRequests).where(and(...conditions)).orderBy(desc(icmChangeRequests.createdAt), desc(icmChangeRequests.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listSystemConfigs({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(icmSystemConfigs.createdAt, icmSystemConfigs.id, cc)); }
  const results = await db.select().from(icmSystemConfigs).where(and(...conditions)).orderBy(desc(icmSystemConfigs.createdAt), desc(icmSystemConfigs.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listTrainingCompletions({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(icmTrainingCompletions.createdAt, icmTrainingCompletions.id, cc)); }
  const results = await db.select().from(icmTrainingCompletions).where(and(...conditions)).orderBy(desc(icmTrainingCompletions.createdAt), desc(icmTrainingCompletions.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listHypercareSupports({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(icmHypercareSupports.createdAt, icmHypercareSupports.id, cc)); }
  const results = await db.select().from(icmHypercareSupports).where(and(...conditions)).orderBy(desc(icmHypercareSupports.createdAt), desc(icmHypercareSupports.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
