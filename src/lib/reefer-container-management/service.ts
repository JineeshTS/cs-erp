...
export async function listReeferBookings({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(rcmReeferBookings.createdAt, rcmReeferBookings.id, cc)); }
  const results = await db.select().from(rcmReeferBookings).where(and(...conditions)).orderBy(desc(rcmReeferBookings.createdAt), desc(rcmReeferBookings.id)).limit(limit + 1);
...
export async function listTempMonitorings({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(rcmTempMonitorings.createdAt, rcmTempMonitorings.id, cc)); }
  const results = await db.select().from(rcmTempMonitorings).where(and(...conditions)).orderBy(desc(rcmTempMonitorings.createdAt), desc(rcmTempMonitorings.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listPtiInspections({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(rcmPtiInspections.createdAt, rcmPtiInspections.id, cc)); }
  const results = await db.select().from(rcmPtiInspections).where(and(...conditions)).orderBy(desc(rcmPtiInspections.createdAt), desc(rcmPtiInspections.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listPowerManagement({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(rcmPowerManagement.createdAt, rcmPowerManagement.id, cc)); }
  const results = await db.select().from(rcmPowerManagement).where(and(...conditions)).orderBy(desc(rcmPowerManagement.createdAt), desc(rcmPowerManagement.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listColdChainDocs({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(rcmColdChainDocs.createdAt, rcmColdChainDocs.id, cc)); }
  const results = await db.select().from(rcmColdChainDocs).where(and(...conditions)).orderBy(desc(rcmColdChainDocs.createdAt), desc(rcmColdChainDocs.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listBreakdownResponses({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(rcmBreakdownResponses.createdAt, rcmBreakdownResponses.id, cc)); }
  const results = await db.select().from(rcmBreakdownResponses).where(and(...conditions)).orderBy(desc(rcmBreakdownResponses.createdAt), desc(rcmBreakdownResponses.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listTempAlerts({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(rcmTempAlerts.createdAt, rcmTempAlerts.id, cc)); }
  const results = await db.select().from(rcmTempAlerts).where(and(...conditions)).orderBy(desc(rcmTempAlerts.createdAt), desc(rcmTempAlerts.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listClaimAnalytics({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(rcmClaimAnalytics.createdAt, rcmClaimAnalytics.id, cc)); }
  const results = await db.select().from(rcmClaimAnalytics).where(and(...conditions)).orderBy(desc(rcmClaimAnalytics.createdAt), desc(rcmClaimAnalytics.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
