...
export async function listImportClearances({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(ccrImportClearances.createdAt, ccrImportClearances.id, cc)); }
  const results = await db.select().from(ccrImportClearances).where(and(...conditions)).orderBy(desc(ccrImportClearances.createdAt), desc(ccrImportClearances.id)).limit(limit + 1);
...
export async function listExportFilings({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(ccrExportFilings.createdAt, ccrExportFilings.id, cc)); }
  const results = await db.select().from(ccrExportFilings).where(and(...conditions)).orderBy(desc(ccrExportFilings.createdAt), desc(ccrExportFilings.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listTransitProcedures({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(ccrTransitProcedures.createdAt, ccrTransitProcedures.id, cc)); }
  const results = await db.select().from(ccrTransitProcedures).where(and(...conditions)).orderBy(desc(ccrTransitProcedures.createdAt), desc(ccrTransitProcedures.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listDutyCalculations({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(ccrDutyCalculations.createdAt, ccrDutyCalculations.id, cc)); }
...
export async function listAeoCompliances({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(ccrAeoCompliances.createdAt, ccrAeoCompliances.id, cc)); }
...
export async function listIspsCompliances({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(ccrIspsCompliances.createdAt, ccrIspsCompliances.id, cc)); }
...
export async function listPscPreparations({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(ccrPscPreparations.createdAt, ccrPscPreparations.id, cc)); }
  const results = await db.select().from(ccrPscPreparations).where(and(...conditions)).orderBy(desc(ccrPscPreparations.createdAt), desc(ccrPscPreparations.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listImoRegulations({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(ccrImoRegulations.createdAt, ccrImoRegulations.id, cc)); }
  const results = await db.select().from(ccrImoRegulations).where(and(...conditions)).orderBy(desc(ccrImoRegulations.createdAt), desc(ccrImoRegulations.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
