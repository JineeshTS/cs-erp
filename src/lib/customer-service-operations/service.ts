...
export async function listServiceCategories({ tenantId, search, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(csoServiceCategories.createdAt, csoServiceCategories.id, cc)); }
...
export async function listInquiries({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(csoInquiries.createdAt, csoInquiries.id, cc)); }

  const results = await db.select().from(csoInquiries).where(and(...conditions)).orderBy(desc(csoInquiries.createdAt), desc(csoInquiries.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listComplaints({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(csoComplaints.createdAt, csoComplaints.id, cc)); }

  const results = await db.select().from(csoComplaints).where(and(...conditions)).orderBy(desc(csoComplaints.createdAt), desc(csoComplaints.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listServiceRequests({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(csoServiceRequests.createdAt, csoServiceRequests.id, cc)); }

  const results = await db.select().from(csoServiceRequests).where(and(...conditions)).orderBy(desc(csoServiceRequests.createdAt), desc(csoServiceRequests.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listSlaPolicies({ tenantId, search, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(csoSlaPolicies.createdAt, csoSlaPolicies.id, cc)); }

  const results = await db.select().from(csoSlaPolicies).where(and(...conditions)).orderBy(desc(csoSlaPolicies.createdAt), desc(csoSlaPolicies.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listSlaBreaches({ tenantId, search, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(csoSlaBreaches.createdAt, csoSlaBreaches.id, cc)); }

  const results = await db.select().from(csoSlaBreaches).where(and(...conditions)).orderBy(desc(csoSlaBreaches.createdAt), desc(csoSlaBreaches.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listEscalations({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(csoEscalations.createdAt, csoEscalations.id, cc)); }

  const results = await db.select().from(csoEscalations).where(and(...conditions)).orderBy(desc(csoEscalations.createdAt), desc(csoEscalations.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listCommunicationLogs({ tenantId, search, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(csoCommunicationLogs.createdAt, csoCommunicationLogs.id, cc)); }

  const results = await db.select().from(csoCommunicationLogs).where(and(...conditions)).orderBy(desc(csoCommunicationLogs.createdAt), desc(csoCommunicationLogs.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listCustomerFeedback({ tenantId, search, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(csoCustomerFeedback.createdAt, csoCustomerFeedback.id, cc)); }

  const results = await db.select().from(csoCustomerFeedback).where(and(...conditions)).orderBy(desc(csoCustomerFeedback.createdAt), desc(csoCustomerFeedback.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listKnowledgeArticles({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(csoKnowledgeArticles.createdAt, csoKnowledgeArticles.id, cc)); }

  const results = await db.select().from(csoKnowledgeArticles).where(and(...conditions)).orderBy(desc(csoKnowledgeArticles.createdAt), desc(csoKnowledgeArticles.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listAgentAssignments({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(csoAgentAssignments.createdAt, csoAgentAssignments.id, cc)); }

  const results = await db.select().from(csoAgentAssignments).where(and(...conditions)).orderBy(desc(csoAgentAssignments.createdAt), desc(csoAgentAssignments.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listResolutionNotes({ tenantId, search, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(csoResolutionNotes.createdAt, csoResolutionNotes.id, cc)); }

  const results = await db.select().from(csoResolutionNotes).where(and(...conditions)).orderBy(desc(csoResolutionNotes.createdAt), desc(csoResolutionNotes.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
