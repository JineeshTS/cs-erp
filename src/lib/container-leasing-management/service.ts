...
export async function listLeaseAgreements({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(clmLeaseAgreements.createdAt, clmLeaseAgreements.id, cc)); }
  const results = await db.select().from(clmLeaseAgreements).where(and(...conditions)).orderBy(desc(clmLeaseAgreements.createdAt), desc(clmLeaseAgreements.id)).limit(limit + 1);
...
export async function listOnhireOffhires({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(clmOnhireOffhires.createdAt, clmOnhireOffhires.id, cc)); }
  const results = await db.select().from(clmOnhireOffhires).where(and(...conditions)).orderBy(desc(clmOnhireOffhires.createdAt), desc(clmOnhireOffhires.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listMnrDamageBillings({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(clmMnrDamageBillings.createdAt, clmMnrDamageBillings.id, cc)); }
  const results = await db.select().from(clmMnrDamageBillings).where(and(...conditions)).orderBy(desc(clmMnrDamageBillings.createdAt), desc(clmMnrDamageBillings.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listLeaseCostAllocations({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(clmLeaseCostAllocations.createdAt, clmLeaseCostAllocations.id, cc)); }
  const results = await db.select().from(clmLeaseCostAllocations).where(and(...conditions)).orderBy(desc(clmLeaseCostAllocations.createdAt), desc(clmLeaseCostAllocations.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listLessorReconciliations({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(clmLessorReconciliations.createdAt, clmLessorReconciliations.id, cc)); }
  const results = await db.select().from(clmLessorReconciliations).where(and(...conditions)).orderBy(desc(clmLessorReconciliations.createdAt), desc(clmLessorReconciliations.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listContainerRedeliveries({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(clmContainerRedeliveries.createdAt, clmContainerRedeliveries.id, cc)); }
  const results = await db.select().from(clmContainerRedeliveries).where(and(...conditions)).orderBy(desc(clmContainerRedeliveries.createdAt), desc(clmContainerRedeliveries.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listLeaseVsBuyAnalyses({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(clmLeaseVsBuyAnalyses.createdAt, clmLeaseVsBuyAnalyses.id, cc)); }
  const results = await db.select().from(clmLeaseVsBuyAnalyses).where(and(...conditions)).orderBy(desc(clmLeaseVsBuyAnalyses.createdAt), desc(clmLeaseVsBuyAnalyses.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listFleetOptimizers({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(clmFleetOptimizers.createdAt, clmFleetOptimizers.id, cc)); }
  const results = await db.select().from(clmFleetOptimizers).where(and(...conditions)).orderBy(desc(clmFleetOptimizers.createdAt), desc(clmFleetOptimizers.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
