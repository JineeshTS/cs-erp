...
export async function listProformaEstimates({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(pdaProformaEstimates.createdAt, pdaProformaEstimates.id, cc)); }
  const results = await db.select().from(pdaProformaEstimates).where(and(...conditions)).orderBy(desc(pdaProformaEstimates.createdAt), desc(pdaProformaEstimates.id)).limit(limit + 1);
...
export async function listFinalDas({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(pdaFinalDas.createdAt, pdaFinalDas.id, cc)); }
  const results = await db.select().from(pdaFinalDas).where(and(...conditions)).orderBy(desc(pdaFinalDas.createdAt), desc(pdaFinalDas.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listPortCosts({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(pdaPortCosts.createdAt, pdaPortCosts.id, cc)); }
  const results = await db.select().from(pdaPortCosts).where(and(...conditions)).orderBy(desc(pdaPortCosts.createdAt), desc(pdaPortCosts.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listAgentStatements({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(pdaAgentStatements.createdAt, pdaAgentStatements.id, cc)); }
  const results = await db.select().from(pdaAgentStatements).where(and(...conditions)).orderBy(desc(pdaAgentStatements.createdAt), desc(pdaAgentStatements.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listExpenseAllocations({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(pdaExpenseAllocations.createdAt, pdaExpenseAllocations.id, cc)); }
  const results = await db.select().from(pdaExpenseAllocations).where(and(...conditions)).orderBy(desc(pdaExpenseAllocations.createdAt), desc(pdaExpenseAllocations.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listVarianceAnalyses({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(pdaVarianceAnalyses.createdAt, pdaVarianceAnalyses.id, cc)); }
  const results = await db.select().from(pdaVarianceAnalyses).where(and(...conditions)).orderBy(desc(pdaVarianceAnalyses.createdAt), desc(pdaVarianceAnalyses.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listCostBenchmarks({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(pdaCostBenchmarks.createdAt, pdaCostBenchmarks.id, cc)); }
  const results = await db.select().from(pdaCostBenchmarks).where(and(...conditions)).orderBy(desc(pdaCostBenchmarks.createdAt), desc(pdaCostBenchmarks.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
...
export async function listConsolidatedReports({ tenantId, search, status, cursor, limit = 50 }: ListParams) {
  ...
  if (cursor) { const cc = parseCompoundCursor(cursor); if (cc) conditions.push(cursorCondition(pdaConsolidatedReports.createdAt, pdaConsolidatedReports.id, cc)); }
  const results = await db.select().from(pdaConsolidatedReports).where(and(...conditions)).orderBy(desc(pdaConsolidatedReports.createdAt), desc(pdaConsolidatedReports.id)).limit(limit + 1);
  const hasMore = results.length > limit;
  const data = hasMore ? results.slice(0, limit) : results;
  return { data, meta: { cursor: hasMore ? encodeCompoundCursor(data[data.length - 1].createdAt, data[data.length - 1].id) : undefined, hasMore } };
}
